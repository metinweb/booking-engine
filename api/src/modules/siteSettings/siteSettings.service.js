...
export const updateSetup = asyncHandler(async (req, res) => {
  ...
  const { b2cDomain, b2bDomain } = req.body
  ...
  await Partner.findByIdAndUpdate(partnerId, {
    'branding.siteDomain': b2cDomain,
    'branding.extranetDomain': b2bDomain
  })
...
export const requestSsl = asyncHandler(async (req, res) => {
  const partnerId = getPartnerId(req)
  const { type } = req.body // 'b2c' or 'b2b'
  ...
  const domainField = type === 'b2c' ? 'b2cDomain' : 'b2bDomain'
  const statusField = type === 'b2c' ? 'b2cSslStatus' : 'b2bSslStatus'
