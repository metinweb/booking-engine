  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between p-4 bg-gradient-to-r rounded-xl"
      ...
      <div class="flex items-center gap-3">
        <div class="w-10 h-10 rounded-full flex items-center justify-center"
             :class="formData.status === 'active' ? 'bg-green-500' : formData.status === 'draft' ? 'bg-amber-500' : 'bg-gray-400'">
          <span class="material-icons text-white text-xl">
            {{ formData.status === 'active' ? 'check' : formData.status === 'draft' ? 'edit' : 'pause' }}
          </span>
        </div>
        <div>
          <p class="font-semibold" :class="formData.status === 'active' ? 'text-green-700 dark:text-green-400' : formData.status === 'draft' ? 'text-amber-700 dark:text-amber-400' : 'text-gray-600 dark:text-slate-400'">
            {{ $t(`planning.roomTypes.status.${formData.status}`) }}
          </p>
          <p class="text-xs" :class="formData.status === 'active' ? 'text-green-600 dark:text-green-500' : formData.status === 'draft' ? 'text-amber-600 dark:text-amber-500' : 'text-gray-500 dark:text-slate-500'">
      ...
      <div class="flex gap-2">
        <button
          ...
          :class="formData.status === status
            ? status === 'active' ? 'bg-green-500 text-white shadow-lg shadow-green-500/30'
              : status === 'draft' ? 'bg-amber-500 text-white shadow-lg shadow-amber-500/30'
    ...
    <div class="border border-gray-200 dark:border-slate-700 rounded-lg p-4">
      ...
      <div class="grid grid-cols-2 md:grid-cols-5 gap-4">
        <div>
          <label class="form-label text-sm">{{ $t('planning.roomTypes.occupancySettings.maxAdults') }}</label>
          <input
            v-model.number="formData.occupancy.maxAdults"
            type="number"
            class="form-input"
            min="1"
            max="10"
        ...
        <div>
          <label class="form-label text-sm">{{ $t('planning.roomTypes.occupancySettings.maxChildren') }}</label>
          <input
            v-model.number="formData.occupancy.maxChildren"
            type="number"
            class="form-input"
            min="0"
            max="6"
        ...
        <div>
          <label class="form-label text-sm">{{ $t('planning.roomTypes.occupancySettings.maxInfants') }}</label>
          <input
            v-model.number="formData.occupancy.maxInfants"
            type="number"
            class="form-input"
            min="0"
            max="2"
        ...
        <div>
          <label class="form-label text-sm">{{ $t('planning.roomTypes.occupancySettings.totalMax') }}</label>
          <input
            v-model.number="formData.occupancy.totalMaxGuests"
            type="number"
            class="form-input"
            min="1"
            max="12"
        ...
        <div>
          <label class="form-label text-sm">{{ $t('planning.roomTypes.occupancySettings.baseOccupancy') }}</label>
          <input
            v-model.number="formData.occupancy.baseOccupancy"
            type="number"
            class="form-input"
            min="1"
            max="10"
    ...
    <div class="border border-gray-200 dark:border-slate-700 rounded-lg p-4">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-3">
        <label
          ...
          :class="formData.amenities.includes(amenity.key)
        ...
        >
          ...
          />
          <span class="material-icons text-sm text-gray-600 dark:text-slate-400">{{ amenity.icon }}</span>
...
const languages = ['tr', 'en', 'ru', 'el', 'de', 'es', 'it', 'fr', 'ro', 'bg', 'pt', 'da', 'zh', 'ar', 'fa', 'he', 'sq', 'uk', 'pl', 'az']
...
const amenitiesList = computed(() => [
  ...
  { key: 'dishwasher', icon: 'dishwasher', label: t('planning.roomTypes.amenities.dishwasher') }
])
