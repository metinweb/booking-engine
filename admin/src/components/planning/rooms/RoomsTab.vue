<template>
  <div class="space-y-6">
      ...
      <div v-else-if="filteredRoomTypes.length > 0" class="grid gap-4">
        ...
        >
          ...
          <button
            ...
            :class="roomType.isBaseRoom ? 'text-yellow-500' : 'text-gray-300 dark:text-slate-600 hover:text-yellow-400'"
            :title="roomType.isBaseRoom ? $t('planning.pricing.clickToRemoveBase') : $t('planning.pricing.setAsBase')"
          >
            <span class="material-icons text-xl">{{ roomType.isBaseRoom ? 'star' : 'star_border' }}</span>
          </button>
            ...
            <div class="text-sm text-gray-500 dark:text-slate-400 flex items-center gap-3">
              ...
              <span
                ...
                :class="{
                  ...
                  'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400': roomType.status === 'deleted'
                }"
              >
          ...
          <div v-if="hasBaseRoom" class="flex items-center gap-2 flex-shrink-0">
            <template v-if="!roomType.isBaseRoom">
              <span class="text-xs text-gray-500 dark:text-slate-400">{{ $t('planning.pricing.adjustment') }}:</span>
              <div class="flex items-center">
                <input
                  ...
                  :class="roomType.priceAdjustment > 0 ? 'text-green-600 dark:text-green-400' : roomType.priceAdjustment < 0 ? 'text-red-600 dark:text-red-400' : ''"
                  min="-100"
                  max="500"
                  step="5"
                />
            ...
            <span v-else class="text-xs text-yellow-600 dark:text-yellow-400 font-medium">{{ $t('planning.pricing.baseRoom') }}</span>
          </div>
    ...
    <div>
      <div class="flex items-center justify-between mb-4">
        ...
        <div class="flex items-center gap-2">
          ...
          <button
            ...
            class="flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs transition-colors"
            :class="showDeletedMealPlans
              ? 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400'
              : 'bg-gray-100 text-gray-600 dark:bg-slate-700 dark:text-slate-400 hover:bg-gray-200 dark:hover:bg-slate-600'"
          >
            <span class="material-icons text-sm">{{ showDeletedMealPlans ? 'visibility' : 'visibility_off' }}</span>
            {{ $t('planning.showDeleted') }}
          </button>
      ...
      <!-- Meal Plans List -->
      <div v-else-if="filteredMealPlans.length > 0" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
        <div
          v-for="plan in filteredMealPlans"
          :key="plan._id"
          class="bg-gray-50 dark:bg-slate-700/50 rounded-lg p-3 text-center relative"
          :class="[
            hasBaseMealPlan && plan.isBaseMealPlan ? 'ring-2 ring-yellow-400 dark:ring-yellow-500' : '',
        ...
        >
          ...
          <button
            ...
            :class="plan.isBaseMealPlan ? 'text-yellow-500' : 'text-gray-300 dark:text-slate-600 hover:text-yellow-400'"
            :title="plan.isBaseMealPlan ? $t('planning.pricing.clickToRemoveBase') : $t('planning.pricing.setAsBase')"
          >
            <span class="material-icons text-lg">{{ plan.isBaseMealPlan ? 'star' : 'star_border' }}</span>
          </button>
          ...
          <div v-if="hasBaseMealPlan" class="mt-2">
            <template v-if="!plan.isBaseMealPlan">
              <div class="flex items-center justify-center gap-1">
                <input
                  ...
                  :class="plan.priceAdjustment > 0 ? 'text-green-600 dark:text-green-400' : plan.priceAdjustment < 0 ? 'text-red-600 dark:text-red-400' : ''"
                  min="-50"
                  max="200"
                  step="5"
                />
            ...
            <span v-else class="text-xs text-yellow-600 dark:text-yellow-400 font-medium">{{ $t('planning.pricing.base') }}</span>
          </div>
    ...
    >
      <div class="space-y-3">
        ...
        <div v-if="deleteType === 'roomType' && deleteTarget?.rateCount > 0" class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-3">
          <div class="flex items-start gap-2">
            <span class="material-icons text-blue-500 text-lg">info</span>
            <div class="text-sm">
              <p class="font-medium text-blue-700 dark:text-blue-400">{{ $t('planning.roomTypes.hasRates') }}</p>
        ...
        <div v-if="deleteType === 'mealPlan' && deleteTarget?.rateCount > 0" class="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg p-3">
          <div class="flex items-start gap-2">
            <span class="material-icons text-blue-500 text-lg">info</span>
            <div class="text-sm">
              <p class="font-medium text-blue-700 dark:text-blue-400">{{ $t('planning.mealPlans.hasRates') }}</p>
              <p class="text-blue-600 dark:text-blue-300 mt-1">
...
const toggleRoomTypeStatus = async (roomType, newStatus) => {
  try {
    const response = await planningService.updateRoomType(props.hotel._id, roomType._id, { status: newStatus })
    if (response.success) {
...
const updateRoomTypeAdjustment = async (roomType) => {
  try {
    await planningService.updateRoomTypePriceAdjustment(props.hotel._id, roomType._id, roomType.priceAdjustment)
...
const updateMealPlanAdjustment = async (plan) => {
  try {
    await planningService.updateMealPlanPriceAdjustment(props.hotel._id, plan._id, plan.priceAdjustment)
    // No toast - inline edit feedback is sufficient
  } catch (error) {
    toast.error(error.response?.data?.message || t('common.operationFailed'))
  }
  ...
  try {
    if (deleteType.value === 'roomType') {
      ...
      if (deleteTarget.value.rateCount > 0) {
        await planningService.updateRoomType(props.hotel._id, deleteTarget.value._id, { status: 'deleted' })
      ...
      if (deleteTarget.value.rateCount > 0) {
        await planningService.updateMealPlan(props.hotel._id, deleteTarget.value._id, { status: 'deleted' })
