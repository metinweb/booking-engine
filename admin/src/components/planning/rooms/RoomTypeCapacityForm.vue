    ...
    <div class="border border-gray-200 dark:border-slate-700 rounded-lg p-4">
      <div class="flex flex-col md:flex-row md:items-center gap-4">
        <div class="flex items-center gap-3">
          ...
          <div>
            <label class="text-sm font-medium text-gray-700 dark:text-slate-300">Fiyatlandırma Tipi</label>
            <p class="text-xs text-gray-500 dark:text-slate-400">Bu oda için kullanılacak fiyat modeli</p>
          </div>
        </div>
        <div class="flex items-center gap-2 bg-gray-100 dark:bg-slate-700 rounded-lg p-1 ml-0 md:ml-auto">
          <button
            type="button"
            @click="formData.pricingType = 'unit'"
            class="px-4 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2"
            :class="formData.pricingType === 'unit'
          ...
          <button
            ...
            class="px-4 py-2 rounded-md text-sm font-medium transition-all flex items-center gap-2"
            :class="formData.pricingType === 'per_person'
      ...
      <div class="mt-3 p-3 rounded-lg text-sm" :class="formData.pricingType === 'per_person' ? 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-700 dark:text-indigo-300' : 'bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300'">
        <span class="material-icons text-sm align-middle mr-1">info</span>
        <template v-if="formData.pricingType === 'per_person'">
          <strong>Kişi Bazlı (OBP):</strong> Her yetişkin sayısı için ayrı fiyat girilir (1 kişi: 80€, 2 kişi: 100€...). Ekstra yetişkin ve tek kişi indirimi kullanılmaz.
        </template>
        <template v-else>
          <strong>Ünite Bazlı:</strong> Oda bazı fiyat + ekstra kişi ücreti. Standart doluluk için tek fiyat, fazla kişi için ek ücret.
        </template>
      </div>
