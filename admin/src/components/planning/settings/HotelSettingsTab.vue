    ...
    <template v-else>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
        <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-2 flex items-center gap-2">
          ...
          {{ $t('planning.settings.pricingSettings') }}
        </h3>
        <p class="text-sm text-gray-500 dark:text-slate-400 mb-6">{{ $t('planning.settings.pricingSettingsDesc') }}</p>
...
const saveSettings = async () => {
  ...
  try {
    const response = await hotelService.updateHotel(props.hotel._id, {
      ...
      pricingSettings: form.value.pricingSettings,
      policies: {
