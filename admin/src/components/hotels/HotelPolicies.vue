<template>
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden">
      ...
      <div class="px-4 py-3 border-b border-gray-200 dark:border-slate-600 flex items-center justify-between bg-green-50/50 dark:bg-green-900/10">
        <div class="flex items-center gap-3">
          ...
          <div>
            ...
            <span v-if="form.freeCancellation.enabled" class="ml-2 text-xs text-green-600 dark:text-green-400">
      ...
      <div class="divide-y divide-gray-200 dark:divide-slate-600">
        ...
        <div v-if="form.cancellationRules.length === 0" class="px-4 py-8 text-center">
          ...
          <p class="text-xs text-gray-400 dark:text-slate-500">{{ $t('hotels.policies.noRulesHelp') }}</p>
        </div>
    ...
    <details class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden">
      <summary class="px-4 py-3 bg-gray-50 dark:bg-slate-700/50 cursor-pointer flex items-center justify-between hover:bg-gray-100 dark:hover:bg-slate-700">
        <div class="flex items-center gap-2">
          <span class="material-icons text-blue-600 dark:text-blue-400">description</span>
          <h3 class="font-semibold text-gray-800 dark:text-white">{{ $t('hotels.policies.policyTexts') }}</h3>
        </div>
...
// Add a new rule
const addRule = () => {
  const lastDays = form.value.cancellationRules.length > 0
...
const removeRule = (index) => {
  const actualIndex = form.value.cancellationRules.findIndex(
    r => r.daysBeforeCheckIn === sortedRules.value[index].daysBeforeCheckIn &&
