...
async function migrateCities() {
  ...
  while (await cursor.hasNext()) {
    ...
    if (doc.name && typeof doc.name === 'object' && !Array.isArray(doc.name)) {
      ...
      const newName =
        doc.nativeName || doc.name.tr || doc.name.en || Object.values(doc.name).find(v => v) || ''
...
async function migrateDistricts() {
  ...
  console.log(
    `  ✓ Checked ${stats.districts.checked} districts, updated ${stats.districts.updated}`
  )
}
  ...
  while (await cursor.hasNext()) {
    ...
    if (doc.name && typeof doc.name === 'object' && !Array.isArray(doc.name)) {
      ...
      if (options.dryRun) {
        ...
        try {
          await TourismRegion.updateOne({ _id: doc._id }, { $set: { name: newName } })
