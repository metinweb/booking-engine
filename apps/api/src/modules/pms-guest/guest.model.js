...
guestSchema.index({ partner: 1, hotel: 1, email: 1 })
guestSchema.index({ partner: 1, hotel: 1, phone: 1 })
guestSchema.index({ partner: 1, hotel: 1, idNumber: 1 })
guestSchema.index({ partner: 1, hotel: 1, 'statistics.lastStayDate': -1 })
guestSchema.index({ partner: 1, hotel: 1, vipLevel: 1 })
...
guestSchema.statics.findOrCreate = async function (hotelId, guestData) {
  ...
  if (guestData.idNumber) {
    ...
    if (existingByIdNumber) return existingByIdNumber
  }
  ...
  if (guestData.phone) {
    const existingByPhone = await this.findOne({
      hotel: hotelId,
