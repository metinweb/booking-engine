...
export const listHotels = asyncHandler(async (req, res) => {
	...
	const [hotels, total] = await Promise.all([
		Hotel.find(query)
			.select('name slug stars type category address.city address.country location.countryCode images amenities featured stats')
			.sort(sortOption)
			.skip(skip)
			.limit(limitNum)
			.lean(),
...
export const createBooking = asyncHandler(async (req, res) => {
	...
	const booking = new Booking({
		...
		totalChildren,
		totalInfants,
		leadGuest: rooms[0]?.guests?.find(g => g.isLead) || rooms[0]?.guests?.[0] || {
