...
export const searchAvailability = asyncHandler(async (req, res) => {
  ...
  for (const roomType of roomTypes) {
    ...
    // Check capacity
    const maxAdults = roomType.occupancy?.maxAdults || 2
    const _maxChildren = roomType.occupancy?.maxChildren || 2
    const maxTotal = roomType.occupancy?.totalMaxGuests || 4
    const totalPax = adults + children.length
      ...
      try {
        ...
        const optionData = {
          ...
          pricing: {
            ...
            finalTotal: channelPrice, // Channel-specific price (NOT hotelCost!)
            avgPerNight: channelPrice / nights,
            perNight: priceResult.pricing.perNight,
            // Include cost data for admin discount validation
            hotelCost: priceResult.pricing.hotelCost, // Cost to hotel (internal)
            b2bPrice: priceResult.pricing.b2bPrice    // B2B price for agencies
          },
          campaigns:
          ...
          nights,
          // Include non-refundable pricing if market supports it (with channel-specific price)
...
export const createBooking = asyncHandler(async (req, res) => {
  ...
  for (const room of rooms) {
    ...
    // Calculate discount as difference between original and channel price
    const channelDiscount = priceResult.pricing.originalTotal - channelPrice
    ...
    const roomBooking = {
      ...
      pricing: {
        ...
        finalTotal: channelPrice, // Use channel-specific price
        avgPerNight: channelPrice / nights
      },
      dailyBreakdown: priceResult.dailyBreakdown,
