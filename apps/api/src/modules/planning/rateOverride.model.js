...
rateOverrideSchema.methods.hasOverrides = function () {
  ...
  const hasOccupancyPricing =
    this.occupancyPricing &&
    Object.values(
      this.occupancyPricing.toObject ? this.occupancyPricing.toObject() : this.occupancyPricing
    ).some(v => v !== null && v !== undefined)

  // Check if multiplier override has any data
  const hasMultiplierOverride =
    this.multiplierOverride &&
    ((this.multiplierOverride.adultMultipliers &&
      this.multiplierOverride.adultMultipliers.size > 0) ||
      (this.multiplierOverride.childMultipliers &&
        this.multiplierOverride.childMultipliers.size > 0) ||
      (this.multiplierOverride.combinationTable &&
        this.multiplierOverride.combinationTable.length > 0))
