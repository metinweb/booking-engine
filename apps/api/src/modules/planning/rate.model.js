...
rateSchema.index({ hotel: 1, roomType: 1, mealPlan: 1, market: 1, date: 1 }, { unique: true })
...
rateSchema.virtual('isBookable').get(function () {
  return this.status === 'active' && !this.stopSale && this.available > 0
})
 ...
 */
rateSchema.statics.bulkUpdateInRange = async function (
  hotelId,
  startDate,
  endDate,
  updates,
...
rateSchema.statics.getPeriodsView = async function (hotelId, filters = {}) {
  ...
  for (const rate of rates) {
    if (!currentPeriod) {
      ...
      isSameValues(currentPeriod, rate) &&
      isConsecutiveDay(currentPeriod.endDate, rate.date)
    ) {
 ...
 */
rateSchema.statics.toggleStopSale = async function (
  hotelId,
  startDate,
  endDate,
  stopSale,
  ...
  return this.updateMany(query, { $set: { stopSale, stopSaleReason: stopSale ? reason : '' } })
}

/**
 * Update allotment for date range
 */
rateSchema.statics.updateAllotment = async function (
  hotelId,
  startDate,
  endDate,
  allotment,
    ...
    if (this.childPricing && this.childPricing.length > 0) {
      for (const tier of this.childPricing) {
        if (tier.minAge > tier.maxAge) {
          return next(
            new Error(
              `Invalid child pricing tier: minAge (${tier.minAge}) > maxAge (${tier.maxAge})`
            )
          )
        }
      }
