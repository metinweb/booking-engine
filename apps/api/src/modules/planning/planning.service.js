  ...
  for (const act of actions) {
    ...
    switch (action) {
      ...
      case 'set_supplement': {
        ...
        const supplementMealPlanCodes =
          filters?.mealPlans !== 'all' && Array.isArray(filters?.mealPlans) ? filters.mealPlans : []
        ...
        logger.info(
          `set_supplement: updated ${supplementUpdated} rates with ${valueType === 'percentage' ? value + '%' : value + ' TL'} supplement`
        )
        break
      }
...
export const importContractPricing = asyncHandler(async (req, res) => {
  ...
  for (const contractRoom of contractRoomTypes) {
    logger.info(
      `Processing room: contractName="${contractRoom.contractName}", matchedCode="${contractRoom.matchedCode}", capacity=${JSON.stringify(contractRoom.capacity)}`
    )
    ...
    if (existingRoom) {
      ...
      logger.info(
        `updateRoomCapacity=${updateRoomCapacity}, hasCapacity=${!!contractRoom.capacity}`
      )
      if (updateRoomCapacity && contractRoom.capacity) {
        ...
        logger.info(
          `Existing room occupancy: maxAdults=${existingOccupancy.maxAdults}, baseOccupancy=${existingOccupancy.baseOccupancy}, maxChildren=${existingOccupancy.maxChildren}, maxInfants=${existingOccupancy.maxInfants}, totalMaxGuests=${existingOccupancy.totalMaxGuests}`
        )
        ...
        logger.info(
          `New occupancy values: maxAdults=${newMaxAdults}, baseOccupancy=${newBaseOccupancy}, maxChildren=${newMaxChildren}, maxInfants=${newMaxInfants}, totalMaxGuests=${newTotalMaxGuests}`
        )
        ...
        if (needsUpdate) {
          ...
          const updateResult = await RoomType.findByIdAndUpdate(
            existingRoom._id,
            {
          ...
          roomsUpdated++
          logger.info(
            `Updated room capacity: ${mappedCode} → maxAdults:${newMaxAdults}, baseOccupancy:${newBaseOccupancy}, children:${newMaxChildren}, infants:${newMaxInfants}, totalMax:${newTotalMaxGuests}`
          )
        ...
        logger.info(
          `Skipping capacity update: updateRoomCapacity=${updateRoomCapacity}, hasCapacity=${!!contractRoom.capacity}`
        )
      }
  ...
  logger.info(
    `Room/MealPlan processing - rooms created: ${roomsCreated}, updated: ${roomsUpdated}, meal plans created: ${mealPlansCreated}`
  )
  ...
  for (const market of markets) {
    ...
    if (!existingSeason) {
      ...
      seasonsCreated++
      logger.info(
        `Created season: ${fullSeasonCode} (${seasonStartDate?.toISOString().split('T')[0]} - ${seasonEndDate?.toISOString().split('T')[0]})`
      )
    }
  }
    ...
    while ((dateMatch = dateRangePattern.exec(warning)) !== null) {
      ...
      for (const period of periods) {
        ...
        if (overlaps && !periodMinStay[period.code]) {
          ...
          logger.info(
            `Set minStay ${minStayValue} for period ${period.code} from warnings (${period.startDate} - ${period.endDate})`
          )
        }
      }
    }
