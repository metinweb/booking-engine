...
const staySchema = new mongoose.Schema({
  ...
  extras: [{
    ...
    category: {
      type: String,
      enum: ['minibar', 'room_service', 'laundry', 'spa', 'restaurant', 'phone', 'damage', 'other'],
      default: 'other'
    },
...
staySchema.methods.extendStay = async function(newCheckOutDate, additionalAmount = 0) {
  ...
  const newNights = Math.ceil((new Date(newCheckOutDate) - new Date(this.checkInDate)) / (1000 * 60 * 60 * 24))
