...
import { emitTransactionUpdate } from './pmsSocket.js'
...
export const getCashierStats = asyncHandler(async (req, res) => {
  ...
  const [
    ...
    Transaction.aggregate([
      {
        $match: {
          hotel: new (await import('mongoose')).default.Types.ObjectId(hotelId),
...
export const getCurrencies = asyncHandler(async (req, res) => {
  ...
  const hotel = await Hotel.findById(hotelId)
  let activeCurrencies = ['TRY'] // Default

  if (hotel) {
    const settings = await PmsSettings.findOne({ hotel: hotelId })
    if (settings?.cashier?.activeCurrencies?.length > 0) {
      activeCurrencies = settings.cashier.activeCurrencies
    }
  }
