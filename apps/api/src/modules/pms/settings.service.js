...
/**
 * Otel PMS ayarlarını getir (yoksa oluştur)
 */
export const getSettings = asyncHandler(async (req, res) => {
  ...
  const settings = await PmsSettings.getOrCreate(hotel.partner, hotelId)
...
export const updateTaxSettings = asyncHandler(async (req, res) => {
  ...
  if (taxes) {
    // taxTypes array'i tamamen değiştirilmeli
    if (taxes.taxTypes) {
...
export const updateHousekeepingSettings = asyncHandler(async (req, res) => {
  ...
  if (housekeeping) {
    // Array'ler tamamen değiştirilmeli
...
export const resetSettings = asyncHandler(async (req, res) => {
  const { hotelId } = req.params
  const { section } = req.body // Hangi bölüm sıfırlanacak? (general, frontDesk, taxes, vb.)
  ...
  // Mevcut ayarları sil ve yeniden oluştur
