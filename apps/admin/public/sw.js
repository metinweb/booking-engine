...
const CACHEABLE_API_ROUTES = ['/api/pms/hotels/', '/api/planning/room-types']
...
async function handleApiRequest(request) {
  ...
  try {
    ...
    return new Response(JSON.stringify({ success: false, error: 'Cevrimdisi', offline: true }), {
      status: 503,
  ...
  event.waitUntil(
    clients.matchAll({ type: 'window', includeUncontrolled: true }).then(clientList => {
      ...
      if (clients.openWindow) {
        ...
        if (
          notificationData?.type === 'fire' ||
          notificationData?.type === 'human' ||
          notificationData?.type === 'theft'
        ) {
