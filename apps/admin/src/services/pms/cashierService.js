/**
 * PMS Cashier Service
 * Handles cashier/POS operations - shifts and transactions
 */
...
export const getShifts = async (hotelId, params = {}) => {
  const response = await pmsApiClient.get(`/pms/hotels/${hotelId}/cashier/shifts`, { params })
  return response.data
}
...
export const getShift = async (hotelId, shiftId) => {
  const response = await pmsApiClient.get(`/pms/hotels/${hotelId}/cashier/shifts/${shiftId}`)
  return response.data
}
...
export const openShift = async (hotelId, data) => {
  const response = await pmsApiClient.post(`/pms/hotels/${hotelId}/cashier/shifts`, data)
  return response.data
}
...
export const getTransactions = async (hotelId, params = {}) => {
  const response = await pmsApiClient.get(`/pms/hotels/${hotelId}/cashier/transactions`, { params })
  return response.data
}
...
export const createTransaction = async (hotelId, data) => {
  const response = await pmsApiClient.post(`/pms/hotels/${hotelId}/cashier/transactions`, data)
  return response.data
}
...
/**
 * Get cash movement type options for select
 */
export const getCashMovementOptions = () => {
  return Object.entries(CASH_MOVEMENT_INFO)
    .filter(([key]) => !['opening', 'closing', 'sale'].includes(key)) // Exclude automatic types
    .map(([value, info]) => ({
      value,
      label: info.label,
      icon: info.icon
