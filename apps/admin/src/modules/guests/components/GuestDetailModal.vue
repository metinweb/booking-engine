  ...
  <div>
    ...
    <div v-if="guest" class="space-y-6">
      ...
      <div v-if="activeTab === 'profile'" class="space-y-4">
          ...
          <div class="flex items-center gap-3">
            ...
            <div>
              ...
              <div class="flex items-center gap-2 mt-1">
                ...
                <span v-if="guest.isBlacklisted" class="px-2 py-0.5 rounded text-xs font-medium bg-red-100 text-red-700">
                  Kara Listede
                </span>
              </div>
            </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-gray-50 dark:bg-slate-700 rounded-lg p-4">
            <h4 class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase mb-3">Kisisel Bilgiler</h4>
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-slate-400">Dogum Tarihi</span>
                <span class="text-gray-900 dark:text-white">{{ guest.dateOfBirth ? formatDate(guest.dateOfBirth) : '-' }}</span>
              </div>
              <div class="flex justify-between">
              ...
              <div class="flex justify-between">
                ...
                <span class="text-gray-900 dark:text-white">{{ getIdTypeLabel(guest.idType) }}</span>
              </div>
              <div class="flex justify-between">
          ...
          <div class="bg-gray-50 dark:bg-slate-700 rounded-lg p-4">
            <h4 class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase mb-3">Iletisim</h4>
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
              ...
              <div class="flex justify-between">
                ...
                <span class="text-gray-900 dark:text-white truncate max-w-[200px]">{{ guest.email || '-' }}</span>
              </div>
              <div class="flex justify-between">
        ...
        <div class="border border-gray-200 dark:border-slate-600 rounded-lg p-4">
          ...
          <div class="grid grid-cols-5 gap-4 text-center">
            <div>
              <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ guest.statistics?.totalStays || 0 }}</p>
              <p class="text-xs text-gray-500 dark:text-slate-400">Konaklama</p>
            </div>
            <div>
              <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ guest.statistics?.totalNights || 0 }}</p>
              <p class="text-xs text-gray-500 dark:text-slate-400">Gece</p>
            </div>
            <div>
              <p class="text-2xl font-bold text-green-600">{{ formatCurrency(guest.statistics?.totalSpent) }}</p>
              <p class="text-xs text-gray-500 dark:text-slate-400">Harcama</p>
            </div>
            <div>
              <p class="text-2xl font-bold text-gray-900 dark:text-white">{{ guest.statistics?.averageStayLength?.toFixed(1) || 0 }}</p>
              <p class="text-xs text-gray-500 dark:text-slate-400">Ort. Gece</p>
            </div>
            <div>
              <p class="text-2xl font-bold text-gray-900 dark:text-white">
                {{ guest.statistics?.firstStayDate ? formatDate(guest.statistics.firstStayDate) : '-' }}
              </p>
      ...
      <div v-if="activeTab === 'stays'" class="space-y-4">
        ...
        <div v-else-if="stayHistory.length === 0" class="text-center py-8 text-gray-500 dark:text-slate-400">
            ...
            <div class="flex items-center justify-between">
              <div>
                <p class="font-medium text-gray-900 dark:text-white">
                  Oda {{ stay.room?.roomNumber }} - {{ stay.roomType?.name?.tr || stay.roomType?.code }}
                </p>
              ...
              <div class="text-right">
                <p class="font-medium text-gray-900 dark:text-white">{{ formatCurrency(stay.totalAmount) }}</p>
                <span
                  class="px-2 py-0.5 rounded text-xs"
                  :class="stay.status === 'checked_out' ? 'bg-gray-100 text-gray-600' : 'bg-green-100 text-green-600'"
                >
      ...
      <div v-if="activeTab === 'notes'" class="space-y-4">
          ...
          <div class="flex gap-3">
            ...
            <div class="flex flex-col gap-2">
              <label class="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                <input type="checkbox" v-model="noteForm.isImportant" class="rounded" />
                Onemli
              </label>
              <button
                @click="addNote"
        ...
        <div v-if="guest.notes?.length > 0" class="space-y-3">
          <div
            ...
            :class="note.isImportant ? 'bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800' : 'bg-gray-50 dark:bg-slate-700'"
          >
      ...
      <div v-if="activeTab === 'preferences'" class="space-y-4">
        <div class="grid grid-cols-2 gap-4">
          <div class="bg-gray-50 dark:bg-slate-700 rounded-lg p-4">
            <h4 class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase mb-3">Oda Tercihleri</h4>
            <div class="space-y-2 text-sm">
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-slate-400">Kat Tercihi</span>
                <span class="text-gray-900 dark:text-white">{{ getFloorPref(guest.preferences?.floorPreference) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-slate-400">Yatak Tipi</span>
                <span class="text-gray-900 dark:text-white">{{ getBedPref(guest.preferences?.bedType) }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-slate-400">Sigara Odasi</span>
                <span class="text-gray-900 dark:text-white">{{ guest.preferences?.smokingRoom ? 'Evet' : 'Hayir' }}</span>
              </div>
              <div class="flex justify-between">
                <span class="text-gray-500 dark:text-slate-400">Sessiz Oda</span>
                <span class="text-gray-900 dark:text-white">{{ guest.preferences?.quietRoom ? 'Evet' : 'Hayir' }}</span>
              </div>
            </div>
          </div>
          <div class="bg-gray-50 dark:bg-slate-700 rounded-lg p-4">
            <h4 class="text-xs font-medium text-gray-500 dark:text-slate-400 uppercase mb-3">Diger</h4>
            <div class="space-y-2 text-sm">
              <div>
                <span class="text-gray-500 dark:text-slate-400 block mb-1">Alerjiler</span>
                <span class="text-gray-900 dark:text-white">{{ guest.preferences?.allergies?.join(', ') || '-' }}</span>
              </div>
              <div>
                <span class="text-gray-500 dark:text-slate-400 block mb-1">Diyet</span>
                <span class="text-gray-900 dark:text-white">{{ guest.preferences?.dietaryRestrictions?.join(', ') || '-' }}</span>
              </div>
              <div>
                <span class="text-gray-500 dark:text-slate-400 block mb-1">Ozel Istekler</span>
                <span class="text-gray-900 dark:text-white">{{ guest.preferences?.specialRequests || '-' }}</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <Modal v-model="showVipModal" title="VIP Seviyesi" size="sm">
      <div class="space-y-3">
        <label
          ...
          :class="selectedVipLevel === level.value ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-gray-200 dark:border-slate-600'"
        >
          <input type="radio" :value="level.value" v-model="selectedVipLevel" class="hidden" />
          <span class="material-icons" :class="level.iconColor">{{ level.icon }}</span>
          <span class="text-gray-900 dark:text-white">{{ level.label }}</span>
        </label>
      </div>
      <template #footer>
        <button @click="showVipModal = false" class="px-4 py-2 text-gray-600 dark:text-gray-400">Iptal</button>
        <button @click="saveVipLevel" class="px-4 py-2 bg-indigo-600 text-white rounded-lg">Kaydet</button>
      </template>
    </Modal>
    ...
    <Modal v-model="showBlacklistModal" title="Kara Listeye Al" size="sm">
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Neden *</label>
        <textarea
      ...
      <template #footer>
        <button @click="showBlacklistModal = false" class="px-4 py-2 text-gray-600 dark:text-gray-400">Iptal</button>
        <button @click="addToBlacklist" :disabled="!blacklistReason" class="px-4 py-2 bg-red-600 text-white rounded-lg disabled:opacity-50">Kara Listeye Al</button>
      </template>
    </Modal>
    ...
    <Modal v-model="showTagsModal" title="Etiketleri Duzenle" size="sm">
      <div class="space-y-3">
        <div class="flex flex-wrap gap-2">
          <button
            ...
            :class="selectedTags.includes(tag) ? 'bg-indigo-600 text-white border-indigo-600' : 'border-gray-300 dark:border-slate-600 text-gray-700 dark:text-gray-300'"
          >
            {{ tag }}
          </button>
        </div>
      ...
      <template #footer>
        <button @click="showTagsModal = false" class="px-4 py-2 text-gray-600 dark:text-gray-400">Iptal</button>
        <button @click="saveTags" class="px-4 py-2 bg-indigo-600 text-white rounded-lg">Kaydet</button>
      </template>
    </Modal>
...
const getFloorPref = (pref) => ({ low: 'Alt Kat', mid: 'Orta Kat', high: 'Ust Kat', any: 'Farketmez' }[pref] || '-')
const getBedPref = (pref) => ({ single: 'Tek', double: 'Cift', twin: 'Twin', king: 'King', any: 'Farketmez' }[pref] || '-')
