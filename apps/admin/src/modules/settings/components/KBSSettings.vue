  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-slate-400">Kimlik Bildirim Sistemi otomatik gonderim</p>
      </div>
      ...
      <div class="space-y-4">
        <div class="flex items-center justify-between p-4 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-slate-400">Check-in sonrasi otomatik KBS bildirimi</p>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input
              type="checkbox"
        ...
        <div v-if="localSettings.autoSend.enabled" class="flex items-center gap-2 text-sm text-gray-500 dark:text-slate-400">
          <span class="material-icons text-amber-500">info</span>
          <span>{{ localSettings.autoSend.nightStartHour }}:00 - {{ localSettings.autoSend.nightEndHour }}:00 arasinda gonderim yapilmaz</span>
        </div>
      </div>
    </div>
    ...
    <div class="border-t border-gray-200 dark:border-slate-700 pt-6">
      ...
      <div class="flex items-center gap-4">
        ...
        >
          ...
          <span class="material-icons text-sm" v-else>wifi</span>
          Baglantiyi Test Et
        </button>
      ...
      <div v-if="testResult" class="mt-4 p-4 rounded-lg" :class="testResult.success ? 'bg-green-50 dark:bg-green-900/20' : 'bg-red-50 dark:bg-red-900/20'">
        <div class="flex items-start gap-2">
          <span class="material-icons" :class="testResult.success ? 'text-green-600' : 'text-red-600'">
            {{ testResult.success ? 'check_circle' : 'error' }}
          </span>
          <div>
            <p class="font-medium" :class="testResult.success ? 'text-green-800 dark:text-green-300' : 'text-red-800 dark:text-red-300'">
              {{ testResult.message }}
            </p>
