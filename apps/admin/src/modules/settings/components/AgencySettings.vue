...
const saveAgency = async () => {
  if (!form.value.companyName || !form.value.email) return
  ...
  try {
    if (showEditModal.value && editingAgency.value) {
      await agencyService.updateAgency(editingAgency.value._id, form.value)
...
const deleteAgencyConfirm = async () => {
  ...
  deleting.value = true
  try {
    await agencyService.deleteAgency(agencyToDelete.value._id)
