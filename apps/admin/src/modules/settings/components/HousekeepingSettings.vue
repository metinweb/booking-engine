  ...
  <div class="space-y-6">
    ...
    <div class="bg-gray-50 dark:bg-slate-700/50 rounded-lg p-4">
      <div class="flex items-center justify-between mb-4">
        <div>
          <h4 class="font-medium text-gray-900 dark:text-white">Oda Yonetimi</h4>
...
import AddRoomModal from '@/modules/housekeeping/components/AddRoomModal.vue'
...
const loadRooms = async () => {
  ...
  try {
    ...
    toast.error('Odalar yuklenirken hata olustu')
  } finally {
...
const deleteRoomConfirmed = async () => {
  ...
  try {
    ...
    toast.success(`Oda ${roomToDelete.value.roomNumber} silindi`)
    rooms.value = rooms.value.filter(r => r._id !== roomToDelete.value._id)
    showDeleteModal.value = false
    roomToDelete.value = null
  } catch (error) {
    console.error('Error deleting room:', error)
    toast.error(error.response?.data?.message || 'Oda silinirken hata olustu')
  } finally {
