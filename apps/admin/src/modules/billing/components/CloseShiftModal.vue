  ...
  <Modal v-model="show" title="Vardiya Kapat" size="lg" @close="close">
    <div class="space-y-6">
      ...
      <div class="bg-gray-50 dark:bg-slate-700/50 rounded-lg p-4">
        <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Vardiya Ozeti</h4>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div>
            <p class="text-xs text-gray-500 dark:text-slate-400">Vardiya No</p>
            <p class="font-medium text-gray-900 dark:text-white">{{ shift?.shiftNumber }}</p>
          </div>
          <div>
            <p class="text-xs text-gray-500 dark:text-slate-400">Kasiyer</p>
      ...
      <div v-if="discrepancy !== 0">
        ...
        <textarea
          ...
          placeholder="Kasadaki fark icin aciklama girin..."
          required
        ></textarea>
      </div>
...
const submit = async () => {
  ...
  try {
    ...
    toast.success('Vardiya kapatildi')
    emit('closed')
    close()
  } catch (error) {
    toast.error(error.response?.data?.message || 'Vardiya kapatilamadi')
  } finally {
    loading.value = false
  }
}
