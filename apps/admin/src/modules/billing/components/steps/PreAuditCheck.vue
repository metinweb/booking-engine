<template>
  <div class="p-6">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <div>
        <h2 class="text-xl font-semibold text-gray-900 dark:text-white">On Kontrol</h2>
        <p class="text-sm text-gray-500 dark:text-slate-400">
          Audit baslamadan once sistemin durumunu kontrol ediyoruz
        </p>
      </div>
    </div>
    ...
    <div v-else class="space-y-4">
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            <h4 class="font-medium text-gray-900 dark:text-white">Bugun Giris Yapacaklar</h4>
      ...
      >
        <div class="flex items-start gap-3">
          ...
          <div class="flex-1 min-w-0">
            <h4 class="font-medium text-gray-900 dark:text-white">Bugun Cikis Yapacaklar</h4>
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">
              <template v-if="checks.housekeeping?.data">
                ...
                <span v-else class="text-amber-600 dark:text-amber-400">
                  {{ checks.housekeeping.data.dirty }} oda temizlik bekliyor
                </span>
              </template>
              <template v-else>Kontrol ediliyor...</template>
            </p>
          ...
          <div class="flex-1 min-w-0">
            ...
            <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">
              <template v-if="checks.shifts?.data">
                ...
                <span v-else>
                  {{ checks.shifts.data.count }} acik kasa
...
const runChecks = async () => {
  ...
  try {
    ...
    toast.error('Kontroller yapilamadi')
...
const handleContinue = async () => {
  if (errorCount.value > 0) {
    toast.error('Lutfen once hatalari cozun')
    return
  }
  ...
  try {
    ...
    if (checks.value.arrivals?.data?.pending > 0) {
      issues.push({
        type: 'arrivals',
        severity: 'warning',
        message: `${checks.value.arrivals.data.pending} bekleyen giris`,
    ...
    if (checks.value.departures?.data?.pending > 0) {
      issues.push({
        type: 'departures',
        severity: 'warning',
        message: `${checks.value.departures.data.pending} bekleyen cikis`,
    ...
    if (checks.value.balances?.data?.count > 0) {
      issues.push({
        type: 'balances',
        severity: 'warning',
        message: `${checks.value.balances.data.count} odada odenmemis bakiye`,
        details: {
    ...
    if (checks.value.housekeeping?.data?.dirty > 0) {
      issues.push({
        type: 'housekeeping',
        severity: 'warning',
        message: `${checks.value.housekeeping.data.dirty} oda temizlik bekliyor`,
