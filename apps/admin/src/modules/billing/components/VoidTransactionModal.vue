    ...
    <div class="space-y-4">
      <div class="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
        ...
        <p class="text-sm text-red-700 dark:text-red-300">
          Bu islem iptal edilecek. Iptal edilen islemler geri alinamaz.
        </p>
      </div>
      ...
      <div>
        <label class="block text-sm text-gray-500 dark:text-slate-400 mb-1">Iptal Nedeni *</label>
        <textarea
          v-model="reason"
          rows="3"
          ...
          placeholder="Iptal nedenini aciklayiniz..."
          required
        ></textarea>
      </div>
    </div>
...
const submit = async () => {
  ...
  try {
    ...
    toast.success('Islem iptal edildi')
    emit('voided')
    close()
  } catch (error) {
    toast.error(error.response?.data?.message || 'Islem iptal edilemedi')
  } finally {
    loading.value = false
  }
}
