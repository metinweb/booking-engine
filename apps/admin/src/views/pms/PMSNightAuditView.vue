  ...
  <div class="space-y-6">
    ...
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-slate-400 mt-1">
          {{ formatDate(auditDate) }} - Gun sonu islemleri
        </p>
      </div>
      ...
      <div class="flex items-center gap-3">
        ...
        <div
          ...
          :title="isConnected ? 'Canli baglanti aktif' : 'Baglanti bekleniyor'"
        >
          ...
          <span>{{ isConnected ? 'Canli' : 'Baglaniyor' }}</span>
        </div>
    ...
    <div v-else-if="!audit" class="bg-white dark:bg-slate-800 rounded-2xl border border-gray-200 dark:border-slate-700 p-8 md:p-12 text-center">
      ...
      >
        ...
        {{ starting ? 'Baslatiliyor...' : 'Audit Baslat' }}
      </button>

      <!-- Quick stats preview -->
      <div class="mt-8 pt-8 border-t border-gray-200 dark:border-slate-700">
        <p class="text-sm text-gray-500 dark:text-slate-400 mb-4">Bugunun ozeti</p>
        <div class="grid grid-cols-2 md:grid-cols-4 gap-4 max-w-2xl mx-auto">
          ...
          <div class="text-center">
            <p class="text-2xl font-bold text-gray-900 dark:text-white">--</p>
            <p class="text-xs text-gray-500 dark:text-slate-400">Gelir</p>
          </div>
        </div>
      </div>
    </div>
                  ...
                  <div class="flex items-center gap-3">
                    <div class="text-right">
                      <p class="text-sm font-medium text-green-600 dark:text-green-400">
                        %{{ item.summary?.occupancyRate || 0 }} Doluluk
                      </p>
...
const PreAuditCheck = defineAsyncComponent(() => import('@/components/pms/nightAudit/steps/PreAuditCheck.vue'))
const NoShowProcessing = defineAsyncComponent(() => import('@/components/pms/nightAudit/steps/NoShowProcessing.vue'))
const RoomChargePosting = defineAsyncComponent(() => import('@/components/pms/nightAudit/steps/RoomChargePosting.vue'))
const CashierReconciliation = defineAsyncComponent(() => import('@/components/pms/nightAudit/steps/CashierReconciliation.vue'))
const ReportsAndClose = defineAsyncComponent(() => import('@/components/pms/nightAudit/steps/ReportsAndClose.vue'))
...
const { auditProgress, isConnected } = useNightAuditSocket(hotelId, {
  ...
  onCompleted: (data) => {
    ...
    fetchAudit()
    toast.success('Night Audit tamamlandi!')
  }
})
...
const fetchAudit = async () => {
  ...
  try {
    ...
    toast.error('Audit bilgisi alinamadi')
  } finally {
    loading.value = false
  }
}
...
const handleStartAudit = async () => {
  ...
  if (!hotelId.value) {
    ...
    toast.warning('Lutfen once bir otel secin')
    return
  }
  ...
  try {
    ...
    audit.value = response.data
    toast.success('Night Audit baslatildi')
  } catch (error) {
    console.error('Failed to start audit:', error)
    toast.error(error.response?.data?.message || 'Audit baslatilamadi')
  } finally {
...
const handleStepComplete = async (data) => {
  ...
  if (stepInfo) {
    toast.success(`${stepInfo.label} tamamlandi`)
  }
}
