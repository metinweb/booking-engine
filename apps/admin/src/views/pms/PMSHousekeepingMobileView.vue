...
const handleLogout = async () => {
  await pmsAuthStore.logout()
  router.push('/pms/login')
}
...
const handleHousekeepingUpdate = (data) => {
  ...
  if (room) {
    room.housekeepingStatus = data.status
    toast.info(t('housekeeping.mobile.statusUpdated', { room: data.roomNumber, status: HOUSEKEEPING_STATUS_INFO[data.status]?.label || data.status }))
  }
}
...
const handleCheckOutEvent = (data) => {
  ...
  toast.warning(t('housekeeping.mobile.checkoutAlert', { room: data.roomNumber }))
...
onMounted(() => {
  ...
  if (pmsAuthStore.user?.hotels?.length > 1) {
    hotels.value = pmsAuthStore.user.hotels
  }
})
