<template>
  <div>
    ...
    <div class="mb-6 flex items-center justify-between">
      <router-link to="/admin/hotel-base" class="inline-flex items-center text-gray-600 dark:text-slate-400 hover:text-purple-600 dark:hover:text-purple-400">
    ...
    <div class="mb-4 p-4 bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-200 dark:border-indigo-800 rounded-lg">
      <div class="flex items-start gap-3">
        ...
        <div>
          <h4 class="font-medium text-indigo-800 dark:text-indigo-300">{{ $t('hotels.hotelBase.baseHotelNotice') }}</h4>
...
const fetchHotel = async () => {
  ...
  loading.value = true
  try {
    const response = await hotelService.getBaseHotel(route.params.id)
    if (response.success) {
...
const handleSave = async () => {
  ...
  try {
    let response
    if (isNew.value) {
      response = await hotelService.createBaseHotel(data)
      if (response.success) {
        toast.success(t('hotels.createSuccess'))
        router.push(`/admin/hotel-base/${response.data._id}`)
      }
    } else {
      response = await hotelService.updateBaseHotel(route.params.id, data)
      if (response.success) {
        hotel.value = {
          ...hotel.value,
          ...response.data,
