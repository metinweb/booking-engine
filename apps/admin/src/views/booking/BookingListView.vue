		...
		<div class="flex-1 overflow-y-auto py-6">
			...
			<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 mb-6">
				<div class="flex flex-col lg:flex-row lg:items-center gap-4">
					...
					<!-- Date Filter -->
					<select v-model="filters.dateRange" @change="applyFilters" class="form-input w-auto">
			...
			<!-- Empty State -->
			<div v-else-if="bookings.length === 0" class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-12">
				<div class="text-center">
			...
			<div v-else class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
				<div class="overflow-x-auto">
					<table class="w-full">
						<thead class="bg-gray-50 dark:bg-slate-700/50">
							<tr>
								<th class="px-4 py-3 text-left text-xs font-semibold text-gray-600 dark:text-slate-300 uppercase tracking-wider">
								...
								<th class="px-4 py-3 text-right text-xs font-semibold text-gray-600 dark:text-slate-300 uppercase tracking-wider">
									{{ $t('common.actions') }}
								</th>
							</tr>
						</thead>
							...
							>
									...
									<div class="flex items-center">
										...
										<div>
											<div class="flex items-center">
												...
												>
													...
													<div class="absolute left-0 top-full mt-1 z-50 hidden group-hover/amend:block">
														<div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-gray-200 dark:border-slate-700 p-3 min-w-[280px]">
															<div class="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-2 flex items-center">
																<span class="material-icons text-amber-500 text-sm mr-1">history</span>
																{{ $t('booking.amendmentHistory') }} ({{ booking.modifications.length }})
															</div>
															<div
											...
											<div class="mt-1 flex items-center gap-1.5">
												...
												>
													<span class="w-1.5 h-1.5 rounded-full mr-1" :class="getStatusDotClass(booking.status)"></span>
													{{ getStatusLabel(booking.status) }}
												</span>
												<!-- Expiry warning for drafts -->
												<span v-if="booking.status === 'draft' && booking.expiresAt" class="text-xs text-orange-500">
								...
								<td class="px-4 py-4">
									<div v-if="booking.leadGuest" class="max-w-[160px]">
										...
										<div v-if="booking.leadGuest.nationality" class="flex items-center gap-1 mt-0.5">
											<span class="text-sm">{{ getCountryFlag(booking.leadGuest.nationality) }}</span>
											<span class="text-xs text-gray-500 dark:text-slate-400">{{ getCountryName(booking.leadGuest.nationality) }}</span>
										</div>
									</div>
								...
								<td class="px-4 py-4">
									...
									<div v-if="booking.rooms?.length" class="relative group">
										...
										<div class="absolute left-0 top-full mt-1 z-50 hidden group-hover:block">
											<div class="bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-gray-200 dark:border-slate-700 p-3 min-w-[280px]">
												<div class="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-2">
												...
												>
													<div class="flex items-start justify-between gap-3">
														...
														<div class="flex items-center gap-1.5 text-xs text-gray-600 dark:text-slate-300">
															...
															<span v-if="getRoomChildren(room) > 0" class="flex items-center" :title="$t('booking.children')">
													...
													<div class="flex items-center gap-1 mt-1.5">
														<span class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-purple-50 dark:bg-purple-900/30 text-purple-600 dark:text-purple-300">
															{{ room.roomTypeCode }}
														</span>
														<span class="px-1.5 py-0.5 rounded text-[10px] font-medium bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-300">
															{{ room.mealPlanCode }}
														</span>
														<span
								...
								<td class="px-4 py-4 whitespace-nowrap">
									<div class="flex flex-wrap gap-1">
										...
										<span
											...
											:style="{
												backgroundColor: booking.season.color ? `${booking.season.color}20` : '#fef3c7',
										...
										<span v-if="!booking.market && !booking.season" class="text-gray-400 dark:text-slate-500 text-sm">-</span>
									</div>
								</td>
								...
								<td class="px-4 py-4 whitespace-nowrap">
									<div v-if="booking.pricing?.grandTotal">
										...
										<p v-if="booking.pricing?.totalCost" class="text-xs text-gray-500 dark:text-slate-400 mt-0.5">
											{{ $t('booking.cost') }}: {{ formatPrice(booking.pricing.totalCost, booking.pricing.currency) }}
										</p>
									</div>
									<span v-else class="text-gray-400 dark:text-slate-500 italic text-sm">-</span>
								</td>
								...
								<td class="px-4 py-4 whitespace-nowrap" @click.stop>
									...
									>
										...
										<div class="flex items-center gap-2 mt-1">
											...
											<span class="material-icons text-xs text-gray-400 group-hover:text-purple-500 transition-colors">open_in_new</span>
										</div>
									</button>
									<span v-else class="text-gray-400 dark:text-slate-500 italic text-sm">-</span>
								</td>
								...
								<td class="px-4 py-4 whitespace-nowrap text-right" @click.stop>
									<div class="flex items-center justify-end gap-1">
										...
										<!-- More Actions Menu -->
										<div class="relative" v-if="booking.status !== 'draft'">
											<button
				...
				<!-- Pagination -->
				<div v-if="totalPages > 1" class="px-4 py-3 bg-gray-50 dark:bg-slate-700/30 border-t border-gray-200 dark:border-slate-700 flex items-center justify-between">
					<div class="text-sm text-gray-500 dark:text-slate-400">
						{{ $t('common.showing') }}
						<span class="font-medium text-gray-900 dark:text-white">{{ (currentPage - 1) * perPage + 1 }}</span>
						-
						<span class="font-medium text-gray-900 dark:text-white">{{ Math.min(currentPage * perPage, totalItems) }}</span>
						{{ $t('common.of') }}
		...
		<Modal v-model="showDeleteModal" :title="$t('booking.deleteDraft')">
			...
			<template #footer>
				<div class="flex justify-end gap-3">
					<button @click="showDeleteModal = false" class="btn-secondary px-4 py-2">
						{{ $t('common.cancel') }}
					</button>
					<button @click="confirmDeleteDraft" class="btn-danger px-4 py-2">
						{{ $t('common.delete') }}
					</button>
				</div>
			</template>
...
const applyDateFilter = (params) => {
	...
	switch (filters.value.dateRange) {
		...
		case 'month':
			params.checkInFrom = today.toISOString()
			params.checkInTo = new Date(today.getFullYear(), today.getMonth() + 1, today.getDate()).toISOString()
			break
		case 'upcoming':
			params.checkInFrom = today.toISOString()
			break
