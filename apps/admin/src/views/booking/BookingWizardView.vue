	...
	<div class="h-full flex flex-col">
		...
		<Modal v-model="showContinueModal" :title="$t('booking.existingDataFound')">
			<div class="py-4">
				...
				<p v-if="existingDraftNumber" class="text-center text-sm text-purple-600 dark:text-purple-400 font-medium mb-4">
					{{ $t('booking.draftNumber') }}: {{ existingDraftNumber }}
				</p>
				<p v-else-if="hasLocalStorageData" class="text-center text-sm text-gray-500 dark:text-slate-500 mb-4">
...
watch(() => route.params.bookingNumber, (newVal, oldVal) => {
	if (newVal !== oldVal) {
		initializeBooking()
	}
})
...
watch(() => bookingStore.draftBookingNumber, (newVal) => {
