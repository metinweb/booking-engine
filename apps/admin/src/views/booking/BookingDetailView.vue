			...
			<div class="hidden print:block print-header mb-6 pb-4 border-b-2 border-gray-300">
				<div class="flex justify-between items-start">
					...
					<div class="text-right">
						<p class="text-lg font-bold text-gray-900">{{ booking?.bookingNumber || booking?.confirmationNumber }}</p>
		...
		<template v-else>
			...
			<div class="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-6">
				...
				<div class="flex flex-wrap gap-2 print:hidden">
					...
					<button @click="printBooking" class="btn-secondary">
			...
			<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 print:block print:space-y-4">
				...
				<div class="lg:col-span-2 space-y-6">
					...
					<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
						...
						<div class="flex items-start space-x-4">
							<div class="w-20 h-20 rounded-lg bg-gray-100 dark:bg-slate-700 flex items-center justify-center flex-shrink-0">
					...
					<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
						...
						<div class="grid grid-cols-2 md:grid-cols-4 gap-3 text-sm">
							<div class="flex">
								<span class="text-gray-500 dark:text-slate-400 min-w-[70px]">{{ $t('booking.checkIn') }}:</span>
								<span class="font-medium text-gray-900 dark:text-white">{{ formatDate(booking.checkIn) }}</span>
							</div>
							<div class="flex">
								<span class="text-gray-500 dark:text-slate-400 min-w-[70px]">{{ $t('booking.checkOut') }}:</span>
								<span class="font-medium text-gray-900 dark:text-white">{{ formatDate(booking.checkOut) }}</span>
							</div>
							<div class="flex">
								<span class="text-gray-500 dark:text-slate-400 min-w-[70px]">{{ $t('booking.nights') }}:</span>
								<span class="font-medium text-gray-900 dark:text-white">{{ booking.nights || nights }} {{ $t('booking.nightsUnit') }}</span>
							</div>
							<div class="flex">
								<span class="text-gray-500 dark:text-slate-400 min-w-[70px]">{{ $t('booking.rooms') }}:</span>
								<span class="font-medium text-gray-900 dark:text-white">{{ booking.rooms?.length || 1 }} {{ $t('booking.roomUnit') }}</span>
							</div>
						</div>
					</div>
								...
								<div class="flex items-start justify-between">
									<div>
										...
										<p class="text-sm text-gray-500 dark:text-slate-400 mt-1">
											{{ room.adults || room.guests?.filter(g => g.type === 'adult').length || 2 }} {{ $t('booking.adults') }}
											<span v-if="(room.children?.length || room.guests?.filter(g => g.type === 'child').length) > 0">
												+ {{ room.children?.length || room.guests?.filter(g => g.type === 'child').length }} {{ $t('booking.children') }}
											</span>
										</p>
									</div>
									<div class="text-right">
										...
										<p v-if="room.nonRefundableDiscount > 0" class="text-xs text-green-600 dark:text-green-400">
											-%{{ room.nonRefundableDiscount }} indirimli
										</p>
									</div>
								</div>
								<!-- Room Guests -->
								<div v-if="room.guests?.length > 0" class="mt-3 pt-3 border-t border-gray-100 dark:border-slate-700">
									<p class="text-xs text-gray-500 dark:text-slate-400 mb-2">{{ $t('booking.roomGuests') }}:</p>
					...
					<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
						...
						<div class="grid grid-cols-1 md:grid-cols-2 gap-3 text-sm">
							<div class="flex">
								<span class="text-gray-500 dark:text-slate-400 min-w-[80px]">{{ $t('booking.name') }}:</span>
								<span class="font-medium text-gray-900 dark:text-white">{{ formatGuestName(booking.leadGuest) }}</span>
							</div>
							<div class="flex">
								<span class="text-gray-500 dark:text-slate-400 min-w-[80px]">{{ $t('booking.email') }}:</span>
								<span class="font-medium text-gray-900 dark:text-white">{{ booking.leadGuest?.email || '-' }}</span>
							</div>
							<div class="flex">
								<span class="text-gray-500 dark:text-slate-400 min-w-[80px]">{{ $t('booking.phone') }}:</span>
								<span class="font-medium text-gray-900 dark:text-white">{{ booking.leadGuest?.phone || '-' }}</span>
							</div>
							<div class="flex">
								<span class="text-gray-500 dark:text-slate-400 min-w-[80px]">{{ $t('booking.nationality') }}:</span>
								<span class="font-medium text-gray-900 dark:text-white">{{ booking.leadGuest?.nationality || '-' }}</span>
							</div>
						</div>
					</div>

					<!-- Special Requests -->
					<div v-if="booking.specialRequests" class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6">
						<h3 class="font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
					...
					<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 print:hidden">
						<h3 class="font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
					...
					<div v-if="amendmentHistory.length > 0 || isLoadingHistory" class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 print:hidden">
						<h3 class="font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
							<span class="material-icons text-purple-500 mr-2">history</span>
							{{ $t('booking.amendmentHistory') }}
							<span v-if="amendmentHistory.length > 0" class="ml-2 px-2 py-0.5 text-xs rounded-full bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400">
				...
				<div class="space-y-6">
					<!-- Price Card -->
					<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-6 sticky top-6">
						...
						<div class="space-y-3">
							<div class="flex justify-between text-sm">
								...
								<span class="text-gray-900 dark:text-white">
									{{ formatPrice(booking.pricing?.originalTotal || booking.pricing?.subtotal, booking.pricing?.currency) }}
								</span>
							</div>

							<div v-if="booking.pricing?.totalDiscount > 0" class="flex justify-between text-sm">
								<span class="text-green-600 dark:text-green-400">{{ $t('booking.discount') }}</span>
							...
							<div v-if="booking.pricing?.campaigns?.length > 0" class="pt-2">
								<p class="text-xs text-gray-500 dark:text-slate-400 mb-1">{{ $t('booking.appliedCampaigns') }}:</p>
							...
							<div class="flex justify-between">
								<span class="font-semibold text-gray-900 dark:text-white">{{ $t('booking.total') }}</span>
								<span class="text-xl font-bold text-purple-600 dark:text-purple-400">
									{{ formatPrice(booking.pricing?.grandTotal || booking.pricing?.finalTotal || booking.pricing?.total, booking.pricing?.currency) }}
								</span>
							</div>

							<div v-if="booking.nights > 0" class="text-right text-xs text-gray-500 dark:text-slate-400">
								~{{ formatPrice((booking.pricing?.grandTotal || booking.pricing?.finalTotal) / booking.nights, booking.pricing?.currency) }} / {{ $t('booking.perNight') }}
							</div>
						</div>

						<!-- Source Info -->
						<div v-if="booking.source" class="mt-6 pt-4 border-t border-gray-200 dark:border-slate-700 print:hidden">
							<p class="text-xs text-gray-500 dark:text-slate-400">{{ $t('booking.source') }}</p>
							<p class="text-sm text-gray-900 dark:text-white">{{ getSourceText(booking.source) }}</p>
						</div>
					</div>
				</div>
			</div>
		...
		<Modal v-model="showCancelModal" :title="$t('booking.cancelBooking')">
			...
			<template #footer>
				<div class="flex space-x-3">
					...
					<button @click="handleCancel" class="btn-primary bg-red-600 hover:bg-red-700 flex-1" :disabled="isCancelling">
...
const getRoomName = (roomType) => {
	...
	if (typeof name === 'object') {
		return name[locale.value] || name.en || name.tr || Object.values(name)[0] || roomType.code || '-'
	}
...
const handleAddNote = async () => {
	...
	try {
		...
		if (response.success) {
			...
			booking.value.notes.push(response.data.note || {
