<template>
  <div class="h-full flex flex-col">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('common.total') }}</p>
            <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ extras.length }}</p>
          </div>
        </div>
      </div>
      <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('common.active') }}</p>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('extra.extraCategories.activity') }}</p>
            <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ categoryCount('activity') }}</p>
          </div>
        </div>
      </div>
      <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('extra.extraCategories.meal') }}</p>
            <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ categoryCount('meal') }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-else-if="filteredExtras.length === 0" class="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 p-12 text-center">
      <span class="material-icons text-6xl text-gray-300 dark:text-gray-600 mb-4">extension_off</span>
    ...
    <div v-else class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      ...
      >
        ...
        <div class="p-4 border-b border-gray-200 dark:border-slate-700">
          <div class="flex items-start justify-between">
            <div class="flex items-center">
              ...
              >
                <span class="material-icons text-white">{{ getCategoryIcon(extra.category) }}</span>
              </div>
              <div>
                <h3 class="font-medium text-gray-900 dark:text-white">{{ getLocalizedName(extra.name) }}</h3>
                <p class="text-sm text-gray-500">{{ extra.code }}</p>
              </div>
            </div>
            <StatusBadge
              :status="extra.status"
              :statusMap="extraStatusMap"
            />
          </div>
        </div>
        ...
        <div class="p-4">
          ...
          <div class="space-y-2">
            <div class="flex items-center justify-between text-sm">
              <span class="text-gray-500 dark:text-gray-400">{{ $t('extra.fields.category') }}</span>
              <span class="font-medium text-gray-900 dark:text-white">
                {{ $t(`extra.extraCategories.${extra.category}`) }}
              </span>
            </div>
            <div class="flex items-center justify-between text-sm">
              <span class="text-gray-500 dark:text-gray-400">{{ $t('extra.fields.priceType') }}</span>
              <span class="font-medium text-gray-900 dark:text-white">
        ...
        <!-- Extra Actions -->
        <div class="p-4 bg-gray-50 dark:bg-slate-700/50 border-t border-gray-200 dark:border-slate-700">
    ...
    <!-- Extra Modal -->
    <Modal v-model="showModal" :title="editingExtra ? $t('extra.editExtra') : $t('extra.newExtra')" size="lg">
      <form @submit.prevent="saveExtra" class="space-y-6">
        ...
        <div>
          ...
          <div v-if="!applyToAllTours" class="border border-gray-200 dark:border-slate-600 rounded-lg max-h-40 overflow-y-auto">
            <div
              v-for="tour in tours"
              :key="tour._id"
            ...
            >
              <input
                ...
                class="h-4 w-4 text-purple-600 focus:ring-purple-500 border-gray-300 rounded"
              />
              <label :for="`tour-${tour._id}`" class="ml-2 text-sm text-gray-700 dark:text-gray-300">
                {{ tour.name }} ({{ tour.code }})
              </label>
            </div>
          </div>
