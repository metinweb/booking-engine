<template>
  <div class="h-full flex flex-col">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
      <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('stats.upcomingDepartures') }}</p>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('departure.departureStatuses.confirmed') }}</p>
      ...
      <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <div class="flex items-center">
          <div class="p-3 rounded-full bg-yellow-100 dark:bg-yellow-900/30">
            <span class="material-icons text-yellow-600 dark:text-yellow-400">hourglass_empty</span>
          </div>
          <div class="ml-4">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('departure.departureStatuses.scheduled') }}</p>
            <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ stats.scheduled }}</p>
          </div>
        </div>
      </div>
      <div class="bg-white dark:bg-slate-800 rounded-lg p-4 border border-gray-200 dark:border-slate-700">
        <div class="flex items-center">
          ...
          <div class="ml-4">
            <p class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ $t('departure.departureStatuses.sold_out') }}</p>
            <p class="text-2xl font-semibold text-gray-900 dark:text-white">{{ stats.soldOut }}</p>
          </div>
        </div>
      </div>
    </div>
    ...
    <!-- Calendar View -->
    <div v-if="viewMode === 'calendar'" class="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 overflow-hidden">
      <!-- Calendar Header -->
        ...
        >
          ...
          <div class="space-y-1">
            <div
              ...
              :title="`${departure.tour?.name || 'Tour'} - ${departure.capacity?.available || 0} ${$t('departure.capacity.available')}`"
            >
              <span class="font-medium">{{ departure.tour?.name || 'Tour' }}</span>
    ...
    <div v-else class="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700">
      ...
      <div v-else-if="filteredDepartures.length === 0" class="p-8 text-center">
        <span class="material-icons text-6xl text-gray-300 dark:text-gray-600 mb-4">event_busy</span>
            ...
            >
              <td class="px-4 py-3">
                <div class="flex items-center">
                  ...
                  <div>
                    <p class="font-medium text-gray-900 dark:text-white">{{ departure.tour?.name || '-' }}</p>
    ...
    <!-- Departure Modal -->
    <Modal v-model="showModal" :title="editingDeparture ? $t('departure.editDeparture') : $t('departure.newDeparture')" size="lg">
      <form @submit.prevent="saveDeparture" class="space-y-6">
        ...
        <div class="border-t border-gray-200 dark:border-slate-700 pt-4">
          <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">{{ $t('departure.pricing.title') }}</h4>
  ...
  try {
    const data = {
      ...
      capacity: {
        ...
        available: departureForm.value.capacity.total - (departureForm.value.capacity.sold || 0) - (departureForm.value.capacity.reserved || 0)
      }
    }
