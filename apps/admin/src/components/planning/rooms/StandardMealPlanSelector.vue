    ...
    <div class="grid gap-3">
      <label
        ...
        :class="[
          ...
          existingCodes.includes(plan.code) ? 'opacity-50 cursor-not-allowed' : ''
        ]"
      >
        <input
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex flex-wrap gap-1 mt-2">
            ...
            <span
              v-if="plan.includedMeals?.alcoholicDrinks"
              class="text-xs bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 px-1.5 py-0.5 rounded"
            >
              {{ $t('planning.mealPlans.meals.alcoholicDrinks') }}
            </span>
            <span
              v-if="!hasAnyMeal(plan)"
...
const handleAdd = async () => {
  ...
  adding.value = true
  try {
    const response = await planningService.addStandardMealPlansToHotel(
      props.hotel._id,
      selectedCodes.value
    )
    if (response.success) {
