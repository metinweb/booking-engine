<template>
  <div class="ai-pricing-assistant">
    ...
    <div
      class="bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/20 dark:to-indigo-900/20 rounded-xl border border-purple-200 dark:border-purple-800 overflow-hidden"
    >
      <!-- Content -->
      <div class="p-4">
        <!-- Command Input Row -->
        <div v-if="!parsedResult" class="flex gap-3 items-start">
          <div
            class="w-9 h-9 rounded-lg bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center shadow-md shadow-purple-500/30 flex-shrink-0"
          >
          ...
          >
            ...
            <span class="hidden sm:inline">{{
              parsing ? $t('planning.pricing.aiAnalyzing') : $t('planning.pricing.aiAnalyze')
            }}</span>
          </button>
        </div>
        ...
        <Transition name="fade">
          <div v-if="hasSelection && !parsedResult" class="flex items-center gap-3 mt-3 ml-12">
            <div
              class="flex items-center gap-2 px-3 py-1.5 bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 rounded-lg"
            >
        ...
        <!-- Example Commands -->
        <div
          v-if="!parsedResult && !command && !hasSelection"
          class="flex flex-wrap gap-2 mt-3 ml-12"
        >
          <button
        ...
        <Transition name="fade">
          <div v-if="parsedResult" class="space-y-4">
            <!-- Error State -->
            <div
              v-if="!parsedResult.success && !parsedResult.needsClarification"
            ...
            <div
              v-else-if="parsedResult.needsClarification"
              ...
              <div class="flex items-start gap-3">
                ...
                <div class="flex-1">
                  <p class="font-medium text-amber-700 dark:text-amber-400">
                    {{ $t('planning.pricing.aiClarificationNeeded') }}
                  </p>
            ...
            >
              ...
              <div
                class="bg-gradient-to-r from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/20 p-4 border-b border-gray-200 dark:border-slate-700"
              >
                <p class="text-gray-600 dark:text-slate-400 text-sm mb-2">
                  {{ $t('planning.pricing.aiCommand') }}: {{ command }}
                </p>
                <div
                  class="flex items-center gap-2 text-emerald-700 dark:text-emerald-400 font-medium"
                >
                ...
                <p class="text-gray-700 dark:text-slate-300 mt-2 font-medium">
                  {{
                    parsedResult.summary?.[locale] ||
                    parsedResult.summary?.tr ||
                    parsedResult.summary?.en
                  }}
                </p>
              </div>
              ...
              <div class="p-4 space-y-3">
                ...
                <div v-if="hasSelection" class="flex items-start gap-3">
                  <span
                    class="w-8 h-8 rounded-lg bg-gradient-to-br from-purple-100 to-indigo-100 dark:from-purple-900/30 dark:to-indigo-900/30 flex items-center justify-center flex-shrink-0 mt-1"
                  >
                  ...
                  <div class="flex-1">
                    <div class="text-xs font-medium text-gray-600 dark:text-slate-400 mb-2">
                      {{ $t('planning.pricing.selectedCells') }}
                    </div>
                ...
                <div v-else class="flex items-start gap-3">
                  <span
                    class="w-8 h-8 rounded-lg bg-gradient-to-br from-blue-100 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 flex items-center justify-center flex-shrink-0 mt-1"
                  >
                    <span class="material-icons text-blue-600 dark:text-blue-400 text-sm"
                      >date_range</span
                    >
                  </span>
                  <div class="flex-1">
                    <div class="flex items-center gap-2 mb-2">
                      <span class="text-xs font-medium text-gray-600 dark:text-slate-400">{{
                        $t('planning.pricing.aiDateRange')
                      }}</span>
                      <span class="text-xs text-gray-400 dark:text-slate-500 italic">{{
                        $t('planning.pricing.aiEditDateHint')
                      }}</span>
                    </div>
                    <DateRangePicker
                ...
                <div class="space-y-2">
                  <span class="text-xs text-gray-500 dark:text-slate-400"
                    >{{ $t('planning.pricing.aiActions') }} ({{
                      parsedResult.actions?.length || 1
                    }})</span
                  >
                  <div class="space-y-2">
                    <div
                    ...
                    >
                      <span
                        ...
                        :class="getActionIconClass(act.action)"
                      >
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <div class="flex flex-wrap gap-1 mt-1">
                          ...
                          <template
                            v-if="
                              act.filters?.roomTypes !== 'all' &&
                              Array.isArray(act.filters?.roomTypes)
                            "
                          >
                            <span
                              v-for="rt in act.filters.roomTypes"
                              :key="rt"
                          ...
                          <template
                            v-if="
                              act.filters?.mealPlans !== 'all' &&
                              Array.isArray(act.filters?.mealPlans)
                            "
                          >
                            <span
                              v-for="mp in act.filters.mealPlans"
                              :key="mp"
                              class="px-1.5 py-0.5 bg-amber-100 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded text-xs"
                            >
                              {{ mp }}
                            </span>
                          </template>
                          ...
                          <template
                            v-if="
                              act.filters?.daysOfWeek !== 'all' &&
                              Array.isArray(act.filters?.daysOfWeek)
                            "
                          >
                            <span
                              class="px-1.5 py-0.5 bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300 rounded text-xs flex items-center gap-1"
                            >
                      ...
                      <div
                        v-if="
                          isValueEditableAction(act.action) &&
                          act.value !== null &&
                          act.value !== undefined
                        "
                      ...
                      >
                        ...
                        <span
                          v-if="act.valueType !== 'percentage'"
                          class="text-emerald-600 dark:text-emerald-400 font-bold text-sm"
                          >TL</span
                        >
                      </div>
                    </div>
              ...
              <!-- Action Buttons -->
              <div
                class="flex items-center justify-between p-4 border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50"
              >
                <button
                ...
                >
                  ...
                  {{
                    executing
                      ? $t('planning.pricing.aiExecuting')
                      : $t('planning.pricing.aiExecute')
                  }}
                </button>
              </div>
            </div>
...
const dateRangeModel = computed({
  ...
  set: val => {
    ...
    if (val.start) {
      parsedResult.value.dateRange.startDate =
        val.start instanceof Date ? val.start.toISOString().split('T')[0] : val.start
    }
    if (val.end) {
      parsedResult.value.dateRange.endDate =
...
const getDateRangeSummary = () => {
  if (!parsedResult.value?.dateRange?.startDate || !parsedResult.value?.dateRange?.endDate)
    return ''
...
const getDaysOfWeekLabel = days => {
  ...
  const isWeekend =
    ...
    normalizedDays.includes('saturday') &&
    normalizedDays.includes('sunday')
  const isWeekday =
    normalizedDays.length === 5 &&
    !normalizedDays.includes('saturday') &&
    !normalizedDays.includes('sunday')
