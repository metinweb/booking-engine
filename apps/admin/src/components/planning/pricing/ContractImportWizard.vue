      ...
      <div class="bg-gray-50 dark:bg-slate-800 rounded-xl p-4">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div class="space-y-3">
            ...
            <label class="flex items-center gap-3 cursor-pointer">
              <input
                v-model="importOptions.createMissingRooms"
                type="checkbox"
            ...
            <label class="flex items-center gap-3 cursor-pointer">
              <input
                v-model="importOptions.createMissingMealPlans"
                type="checkbox"
            ...
            <label class="flex items-center gap-3 cursor-pointer">
              <input
                v-model="importOptions.updateRoomCapacity"
                type="checkbox"
    ...
    <div v-if="currentStep === 3" class="space-y-4">
      ...
      <div v-else class="space-y-4">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
          <!-- Room Capacities -->
          <div
            v-if="roomCapacityChanges.length > 0"
          ...
          >
            ...
            <div class="space-y-1 text-sm">
              <div
                v-for="room in roomCapacityChanges"
                :key="room.code"
          ...
          <div
            v-if="minStayPeriods.length > 0"
            class="bg-orange-50 dark:bg-orange-900/20 rounded-xl p-4"
          >
            <h5
              class="font-medium text-orange-800 dark:text-orange-300 mb-2 flex items-center gap-2"
            >
            ...
            <div class="space-y-1 text-sm">
              <div
                v-for="period in minStayPeriods"
                :key="period.code"
              ...
              >
                ...
                <div class="flex items-center gap-2">
                  ...
                  <span
                    v-if="period.fromWarnings"
                    class="text-xs text-gray-400"
                    title="Uyarılardan çıkarıldı"
                  >
                    (uyarıdan)
                  </span>
                </div>
          ...
          <!-- EB Discounts -->
          <div
            v-if="ebDiscounts.length > 0"
            class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 md:col-span-2"
          >
            ...
            <div class="space-y-2 text-sm">
              <div
                v-for="(eb, index) in ebDiscounts"
                :key="index"
                class="bg-white dark:bg-slate-700 rounded-lg p-3"
              >
                <div class="flex items-center justify-between mb-1">
                  ...
                  <span
                    class="bg-green-100 dark:bg-green-800 text-green-800 dark:text-green-200 px-2 py-0.5 rounded-full text-xs font-bold"
                  >
                ...
                <div class="grid grid-cols-2 gap-2 text-xs text-gray-500 dark:text-gray-400">
                  <div>
                    ...
                    {{ formatDate(eb.salePeriod?.startDate) }} -
                    {{ formatDate(eb.salePeriod?.endDate) }}
                  </div>
                  <div>
                    <span class="text-gray-400">Konaklama:</span>
                    {{ formatDate(eb.stayPeriod?.startDate) }} -
                    {{ formatDate(eb.stayPeriod?.endDate) }}
                  </div>
                </div>
              </div>
            </div>
          ...
          <div
            v-if="extraPersonPricing.length > 0"
            class="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4 md:col-span-2"
          >
            <h5
              class="font-medium text-purple-800 dark:text-purple-300 mb-2 flex items-center gap-2"
            >
            ...
            <div class="overflow-x-auto">
              <table class="w-full text-xs">
                ...
                <tbody>
                  <tr
                    v-for="(price, index) in extraPersonPricing"
                    :key="index"
  ...
  return parsedData.value.earlyBookingDiscounts
    ...
    .map(eb => {
      ...
      if (eb.stayPeriod?.startDate) {
        ...
        stayPeriod = {
          startDate:
            parsedData.value.contractInfo?.validFrom ||
            seasonStartDate.value?.toISOString().split('T')[0],
          endDate:
            parsedData.value.contractInfo?.validTo ||
            seasonEndDate.value?.toISOString().split('T')[0]
        }
      }
...
const extraPersonPricing = computed(() => {
  ...
  const periodMap = {}
  periods.forEach(p => {
    periodMap[p.code] = p.name || p.code
  })

  const roomMap = {}
  rooms.forEach(r => {
    roomMap[r.contractCode || r.suggestedCode] = r.contractName
  })
