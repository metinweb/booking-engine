<template>
  <form @submit.prevent="handleSubmit" class="space-y-6">
    ...
    <div>
      <h3 class="text-lg font-semibold text-gray-800 dark:text-white mb-2">{{ $t('hotels.seo.title') }}</h3>
    ...
    <div class="pt-6 border-t border-gray-200 dark:border-slate-700">
      ...
      <p class="text-sm text-gray-500 dark:text-slate-400 mb-4">{{ $t('hotels.seo.previewHelp') }}</p>

      <!-- Preview Card -->
      <div class="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-lg p-4 shadow-sm max-w-2xl">
        <div class="space-y-1">
          <h5 class="text-blue-600 dark:text-blue-400 text-lg font-medium hover:underline cursor-pointer truncate">
...
const previewDescription = computed(() => {
  const lang = locale.value
  return form.value.description[lang] || form.value.description.tr || form.value.description.en || ''
})
