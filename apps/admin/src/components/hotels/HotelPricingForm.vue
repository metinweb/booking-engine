    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="md:col-span-2">
        ...
        <div class="flex gap-4 mt-2">
          <label
            class="flex items-center cursor-pointer p-4 border rounded-lg transition-all"
            :class="
              form.model === 'net'
            ...
            "
          >
            <input v-model="form.model" type="radio" value="net" class="form-radio mr-3" />
            <div>
              <div class="font-medium text-gray-800 dark:text-white">
                {{ $t('hotels.pricing.netPrice') }}
              </div>
              <div class="text-sm text-gray-500 dark:text-slate-400">
                {{ $t('hotels.pricing.netPriceDesc') }}
              </div>
            </div>
          </label>
          <label
            ...
            "
          >
            <input v-model="form.model" type="radio" value="rack" class="form-radio mr-3" />
            <div>
              <div class="font-medium text-gray-800 dark:text-white">
                {{ $t('hotels.pricing.rackRate') }}
              </div>
              <div class="text-sm text-gray-500 dark:text-slate-400">
                {{ $t('hotels.pricing.rackRateDesc') }}
              </div>
            </div>
          </label>
        </div>
      ...
      <div
        v-if="form.model === 'net'"
        class="md:col-span-2 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg"
      >
        <div class="flex items-start gap-3">
          <span class="material-icons text-blue-500">info</span>
          <div>
            <h4 class="font-medium text-blue-800 dark:text-blue-300">
              {{ $t('hotels.pricing.netPriceInfo') }}
            </h4>
            <p class="text-sm text-blue-600 dark:text-blue-400 mt-1">
              {{ $t('hotels.pricing.netPriceExample') }}
            </p>
          </div>
        </div>
        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              v-model.number="form.defaultMarkup"
              type="number"
              min="0"
              max="100"
              step="0.1"
      ...
      <div
        v-if="form.model === 'rack'"
        class="md:col-span-2 p-4 bg-amber-50 dark:bg-amber-900/20 rounded-lg"
      >
        <div class="flex items-start gap-3">
          <span class="material-icons text-amber-500">info</span>
          <div>
            <h4 class="font-medium text-amber-800 dark:text-amber-300">
              {{ $t('hotels.pricing.rackRateInfo') }}
            </h4>
            <p class="text-sm text-amber-600 dark:text-amber-400 mt-1">
              {{ $t('hotels.pricing.rackRateExample') }}
            </p>
          </div>
        </div>
        <div class="mt-4 grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              v-model.number="form.defaultCommission"
              type="number"
              min="0"
              max="100"
              step="0.1"
      ...
      <div>
        ...
        <div class="flex items-center gap-4 mt-2">
          <label class="flex items-center cursor-pointer">
            <input v-model="form.taxIncluded" type="checkbox" class="form-checkbox mr-2" />
    ...
    <div class="mt-8 p-6 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
      ...
      <div class="flex items-center justify-center gap-2 flex-wrap">
        <div class="text-center p-3 bg-white dark:bg-slate-800 rounded-lg shadow">
          ...
          <div class="text-xs mt-1 text-gray-600 dark:text-slate-400">
            {{ $t('hotels.pricing.hotelPrice') }}
          </div>
          <div class="font-medium">
            {{
              form.model === 'net' ? $t('hotels.pricing.netPrice') : $t('hotels.pricing.rackRate')
            }}
          </div>
        </div>
        ...
        <div class="text-center p-3 bg-white dark:bg-slate-800 rounded-lg shadow">
          ...
          <div class="text-xs mt-1 text-gray-600 dark:text-slate-400">
            {{ $t('hotels.pricing.partner') }}
          </div>
          <div class="font-medium text-purple-600">
            {{
              form.model === 'net'
                ? `+${form.defaultMarkup || 0}%`
                : `-${form.defaultCommission || 0}%`
            }}
          </div>
        </div>
        <span class="material-icons text-gray-400">arrow_forward</span>
