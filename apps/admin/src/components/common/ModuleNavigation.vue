		...
		<div class="flex items-center gap-1 overflow-x-auto scrollbar-hide py-2">
			...
			>
				...
				<span
					...
					:class="item.badgeClass || 'bg-gray-200 dark:bg-slate-600 text-gray-700 dark:text-slate-300'"
				>
					{{ item.badge }}
				</span>
			</RouterLink>
			...
			<div v-if="!actionsInline && ($slots.actions || actions.length > 0)" class="ml-auto flex items-center gap-2">
				<slot name="actions">
					<template v-for="action in actions" :key="action.name">
						<RouterLink
							v-if="action.to"
...
const colorClasses = {
	purple: {
		...
		inactive: 'text-gray-600 dark:text-slate-400 hover:text-purple-600 dark:hover:text-purple-400 hover:bg-gray-100 dark:hover:bg-slate-700'
	},
	indigo: {
		active: 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400',
		inactive: 'text-gray-600 dark:text-slate-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-gray-100 dark:hover:bg-slate-700'
	},
	blue: {
		active: 'bg-blue-100 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400',
		inactive: 'text-gray-600 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 hover:bg-gray-100 dark:hover:bg-slate-700'
	},
	green: {
		active: 'bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400',
		inactive: 'text-gray-600 dark:text-slate-400 hover:text-green-600 dark:hover:text-green-400 hover:bg-gray-100 dark:hover:bg-slate-700'
	}
}
