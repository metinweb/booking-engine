<template>
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
      ...
      <!-- Country Dropdown (Teleport to body) -->
      <Teleport to="body">
        <Transition
      ...
      <!-- City Dropdown (Teleport to body) -->
      <Teleport to="body">
        <Transition
          enter-active-class="transition ease-out duration-100"
          enter-from-class="transform opacity-0 scale-95"
...
// Dropdown styles for positioning
const countryDropdownStyle = ref({ top: '0px', left: '0px', width: '300px' })
const cityDropdownStyle = ref({ top: '0px', left: '0px', width: '300px' })
const DROPDOWN_HEIGHT = 300

// Update country dropdown position
const updateCountryDropdownPosition = () => {
  if (!countryDropdownRef.value) return

  const rect = countryDropdownRef.value.getBoundingClientRect()
...
const updateCityDropdownPosition = () => {
  if (!cityDropdownRef.value) return

  const rect = cityDropdownRef.value.getBoundingClientRect()
  const viewportHeight = window.innerHeight
