  ...
  <div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-green-600 to-emerald-600 rounded-xl p-6 text-white">
      <div class="flex items-center justify-between">
        <div>
          <h2 class="text-lg font-medium opacity-90">{{ $t('dashboard.agency.monthlyCommission') }}</h2>
          <p class="text-4xl font-bold mt-2">₺{{ (stats.commission?.monthly || 0).toLocaleString('tr-TR') }}</p>
          <p class="text-sm opacity-75 mt-1">{{ $t('dashboard.agency.fromBookings', { count: stats.bookings?.monthly || 0 }) }}</p>
        </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="bg-white dark:bg-slate-800 rounded-xl p-4 text-center shadow">
        ...
        <p class="text-2xl font-bold text-gray-800 dark:text-white mt-2">{{ stats.bookings?.weekly || 0 }}</p>
        <p class="text-sm text-gray-500 dark:text-slate-400">{{ $t('dashboard.agency.weeklyBookings') }}</p>
      </div>
      <div class="bg-white dark:bg-slate-800 rounded-xl p-4 text-center shadow">
        <span class="material-icons text-3xl text-blue-600">people</span>
        <p class="text-2xl font-bold text-gray-800 dark:text-white mt-2">{{ stats.users?.total || 0 }}</p>
        <p class="text-sm text-gray-500 dark:text-slate-400">{{ $t('dashboard.agency.teamMembers') }}</p>
      </div>
      <div class="bg-white dark:bg-slate-800 rounded-xl p-4 text-center shadow">
        ...
        <p class="text-2xl font-bold text-gray-800 dark:text-white mt-2">
          {{ stats.bookings?.monthly && stats.bookings?.total ? Math.round((stats.bookings.monthly / stats.bookings.total) * 100) : 0 }}%
        </p>
        <p class="text-sm text-gray-500 dark:text-slate-400">{{ $t('dashboard.agency.monthlyGrowth') }}</p>
      </div>
      <div class="bg-white dark:bg-slate-800 rounded-xl p-4 text-center shadow">
        <span class="material-icons text-3xl text-orange-600">hotel</span>
        <p class="text-2xl font-bold text-gray-800 dark:text-white mt-2">{{ topHotels?.length || 0 }}</p>
        <p class="text-sm text-gray-500 dark:text-slate-400">{{ $t('dashboard.agency.activeHotels') }}</p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl shadow p-6">
        <div class="flex items-center justify-between mb-4">
          ...
          <router-link to="/bookings" class="text-sm text-purple-600 hover:text-purple-700 dark:text-purple-400">
            {{ $t('dashboard.viewAll') }} →
          </router-link>
        </div>
          ...
          >
            <div class="flex items-center">
              ...
              <div>
                ...
                <p class="text-sm text-gray-500 dark:text-slate-400">{{ hotel.count }} {{ $t('dashboard.bookings') }}</p>
              </div>
            </div>
            <p class="font-semibold text-green-600 dark:text-green-400">₺{{ hotel.revenue?.toLocaleString('tr-TR') }}</p>
          </div>
