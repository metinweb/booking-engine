            ...
            <div v-if="sendEmail" class="ml-7 space-y-2 overflow-hidden">
              <div class="flex items-center gap-2">
                ...
                <input
                  v-model="emailInput"
                  type="text"
                  placeholder="E-posta adresleri (virgul ile ayirin)"
    ...
    <div class="mt-6 pt-6 border-t border-gray-200 dark:border-slate-700">
      <div class="flex items-center justify-between">
        <p class="text-sm text-gray-500 dark:text-slate-400">
          {{ formatDate(audit.auditDate) }} tarihi kapatilacak
        </p>
        ...
        <div class="flex items-center gap-3">
          ...
          >
            ...
            {{ completing ? 'Kapatiliyor...' : 'Gunu Kapat ve Yeni Gune Basla' }}
          </button>
        </div>
      </div>
    </div>
...
const fetchSummary = async () => {
  ...
  try {
    ...
    toast.error('Ozet bilgiler yuklenemedi')
  } finally {
    loading.value = false
  }
}
...
const addEmail = () => {
  ...
  for (const email of emails) {
    if (!emailRegex.test(email)) {
      toast.warning(`Gecersiz e-posta: ${email}`)
      continue
    }

    if (emailRecipients.value.includes(email)) {
      toast.warning(`${email} zaten listede`)
      continue
    }
...
const handleDayClose = async () => {
  ...
  if (sendEmail.value && emailRecipients.value.length === 0) {
    toast.warning('E-posta gondermek icin en az bir alici ekleyin')
    return
  }
  ...
  try {
    ...
    toast.error(error.response?.data?.message || 'Gun kapatilamadi')
  } finally {
