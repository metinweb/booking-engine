<template>
  <div class="p-6">
    ...
    <div class="flex items-center gap-3 mb-6">
      ...
      <div>
        ...
        <p class="text-sm text-gray-500 dark:text-slate-400">
          Gunluk kasa kapatma islemini tamamlayin
        </p>
      </div>
    </div>
    ...
    >
      ...
      <p class="text-gray-500 dark:text-slate-400">Acik kasa bulunmuyor, devam edebilirsiniz</p>
    </div>
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                ...
                <p class="text-sm text-gray-500 dark:text-slate-400">
                  {{ shift.shiftNumber }} - Acilis: {{ formatTime(shift.openedAt) }}
                </p>
              </div>
            </div>
            <span
          ...
          <div class="space-y-3">
            ...
            <div class="flex items-center justify-between py-2 text-sm">
              ...
              <span class="text-gray-600 dark:text-gray-400">
                {{ formatCurrency(shift.cardPayments) }} ({{ shift.cardTransactions }} islem)
              </span>
            </div>
          </div>
            ...
            <div class="space-y-4">
              ...
              <div v-if="getDiscrepancy(shift) !== 0">
                ...
                <textarea
                  ...
                  placeholder="Fark nedenini aciklayin..."
                ></textarea>
              </div>
          ...
          <div v-else class="mt-4 p-3 bg-green-50 dark:bg-green-900/20 rounded-lg">
            <div class="flex items-center justify-between">
              <div class="flex items-center gap-2 text-green-700 dark:text-green-400">
                ...
                <span>Kapanis: {{ formatTime(shift.closedAt) }}</span>
              </div>
...
const fetchCashierData = async () => {
  ...
  try {
    ...
    toast.error('Kasa bilgileri yuklenemedi')
  } finally {
    loading.value = false
  }
}
...
const closeShift = async (shift) => {
  ...
  if (discrepancy !== 0 && !input.note) {
    toast.warning('Fark varsa aciklama gereklidir')
    return
  }
  ...
  try {
    ...
    toast.error(error.response?.data?.message || 'Kasa kapatılamadı')
  } finally {
...
const handleContinue = async () => {
  if (shifts.value.length > 0 && !allClosed.value) {
    toast.warning('Lutfen tum kasalari kapatin')
    return
  }
