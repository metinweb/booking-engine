<template>
  ...
  >
    <div class="space-y-6">
      <!-- Room Selection -->
      <div>
        <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Oda Secimi</h4>
        <div class="grid grid-cols-2 gap-4">
          ...
          <div>
            <label class="block text-sm text-gray-500 dark:text-slate-400 mb-1">Musait Oda *</label>
            <select
              v-model="form.roomId"
              class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500"
              required
      ...
      <!-- Main Guest -->
      <div>
        <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">Ana Misafir</h4>
        <div class="grid grid-cols-2 gap-4">
          <div>
...
const fetchData = async () => {
  ...
  try {
    const [roomTypesRes, roomsRes] = await Promise.all([
      planningService.getRoomTypes(props.hotelId),
