<template>
  <Modal
    v-model="show"
    :title="mode === 'bulk' ? 'Toplu Oda Ekle' : 'Yeni Oda Ekle'"
    size="lg"
    @close="close"
  >
    <!-- Mode Tabs -->
    ...
    <div v-if="mode === 'single'" class="space-y-4">
      ...
      <div>
        ...
        <textarea
          ...
          rows="2"
          class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500"
          placeholder="Oda hakkinda notlar..."
        ></textarea>
      </div>
    </div>
    ...
    <div v-else class="space-y-4">
      <div class="bg-blue-50 dark:bg-blue-900/20 rounded-lg p-4">
        <p class="text-sm text-blue-700 dark:text-blue-300">
          Toplu oda ekleme ile belirli bir araliktaki odalari tek seferde olusturabilirsiniz.
          Ornegin: 101'den 110'a kadar olan odalari tek seferde ekleyebilirsiniz.
        </p>
      </div>
      ...
      <div>
        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
          Oda Numarasi Formati
        </label>
        <select
          v-model="bulkRooms.format"
          class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-700 text-gray-900 dark:text-white focus:ring-2 focus:ring-primary-500"
        >
          <option value="number">Sayi (101, 102, 103...)</option>
          <option value="floor-number">Kat-Numara (1-01, 1-02, 1-03...)</option>
        </select>
      </div>
...
const save = async () => {
  ...
  try {
    if (mode.value === 'single') {
      ...
      toast.success(`${rooms.length} oda olusturuldu`)
    }
    emit('created')
    close()
  } catch (error) {
    toast.error(error.response?.data?.message || 'Oda olusturulamadi')
  } finally {
    loading.value = false
  }
}
