  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      ...
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4">
        ...
        <p class="text-2xl font-bold text-blue-600 dark:text-blue-400">{{ data?.summary?.expected || 0 }}</p>
      </div>
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4">
        <p class="text-sm text-gray-500 dark:text-slate-400">Giris Yapti</p>
        <p class="text-2xl font-bold text-green-600 dark:text-green-400">{{ data?.summary?.checkedIn || 0 }}</p>
      </div>
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4">
        <p class="text-sm text-gray-500 dark:text-slate-400">No-Show</p>
        <p class="text-2xl font-bold text-red-600 dark:text-red-400">{{ data?.summary?.noShow || 0 }}</p>
      </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
      <div class="px-4 py-3 border-b border-gray-200 dark:border-slate-700">
        <h3 class="font-medium text-gray-900 dark:text-white">{{ data?.date }} Giris Listesi</h3>
      </div>
        ...
        <table class="w-full">
          ...
          <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
            <tr v-for="arrival in data?.arrivals || []" :key="arrival.stayNumber">
              <td class="px-4 py-3">
                <p class="text-sm font-medium text-gray-900 dark:text-white">{{ arrival.guestName }}</p>
              ...
              <td class="px-4 py-3 text-center text-sm text-gray-900 dark:text-white">{{ arrival.nights }}</td>
              <td class="px-4 py-3">
                <span
