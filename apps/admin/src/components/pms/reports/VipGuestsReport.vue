  ...
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4">
        <p class="text-sm text-gray-500 dark:text-slate-400">Toplam VIP</p>
      ...
      <div class="bg-purple-50 dark:bg-purple-900/20 rounded-xl border border-purple-200 dark:border-purple-800 p-4">
        ...
        <p class="text-2xl font-bold text-purple-700 dark:text-purple-300">{{ data?.summary?.platinum || 0 }}</p>
      </div>
      <div class="bg-yellow-50 dark:bg-yellow-900/20 rounded-xl border border-yellow-200 dark:border-yellow-800 p-4">
        ...
        <p class="text-2xl font-bold text-yellow-700 dark:text-yellow-300">{{ data?.summary?.gold || 0 }}</p>
      </div>
      <div class="bg-gray-50 dark:bg-gray-700 rounded-xl border border-gray-200 dark:border-gray-600 p-4">
        ...
        <p class="text-2xl font-bold text-gray-700 dark:text-gray-300">{{ data?.summary?.silver || 0 }}</p>
      </div>
    </div>

    <!-- Total Spent -->
    <div class="bg-gradient-to-r from-purple-500 to-indigo-500 rounded-xl p-6 text-white">
      <p class="text-sm opacity-80">VIP Misafirler Toplam Harcama</p>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
      <div class="px-4 py-3 border-b border-gray-200 dark:border-slate-700">
        <h3 class="font-medium text-gray-900 dark:text-white">VIP Misafir Listesi</h3>
      </div>
        ...
        <table class="w-full">
          <thead class="bg-gray-50 dark:bg-slate-700/50">
            <tr>
              ...
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Iletisim</th>
              <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">VIP</th>
              <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Konaklama</th>
              <th class="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Gece</th>
              <th class="px-4 py-3 text-right text-xs font-medium text-gray-500 uppercase">Toplam Harcama</th>
              <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Son Konaklama</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-200 dark:divide-slate-700">
            <tr v-for="guest in data?.guests || []" :key="guest.email">
              ...
              <td class="px-4 py-3 text-center text-sm text-gray-900 dark:text-white">{{ guest.totalStays }}</td>
              <td class="px-4 py-3 text-center text-sm text-gray-900 dark:text-white">{{ guest.totalNights }}</td>
              <td class="px-4 py-3 text-right text-sm font-medium text-green-600 dark:text-green-400">{{ formatCurrency(guest.totalSpent) }}</td>
              <td class="px-4 py-3 text-sm text-gray-600 dark:text-gray-400">{{ formatDate(guest.lastStayDate) }}</td>
            </tr>
          </tbody>
        </table>
      </div>
