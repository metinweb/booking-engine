	...
	<div class="border border-gray-200 dark:border-slate-700 rounded-lg overflow-hidden">
		...
		>
			...
			<div class="flex items-center gap-3">
				...
				<span class="material-icons text-gray-400 transition-transform" :class="{ 'rotate-180': isExpanded }">
					expand_more
				</span>
			</div>
		</div>
			...
			<div v-if="pathParams?.length" class="space-y-2">
				...
				<div class="overflow-x-auto">
					<table class="min-w-full text-sm">
						...
						<tbody>
							<tr v-for="param in pathParams" :key="param.name" class="border-t border-gray-100 dark:border-slate-700">
								<td class="py-2 pr-4">
			...
			<div v-if="queryParams?.length" class="space-y-2">
				...
				<div class="overflow-x-auto">
					<table class="min-w-full text-sm">
						...
						<tbody>
							<tr v-for="param in queryParams" :key="param.name" class="border-t border-gray-100 dark:border-slate-700">
								<td class="py-2 pr-4">
									<code class="text-purple-600 dark:text-purple-400">{{ param.name }}</code>
								</td>
								<td class="py-2 pr-4 text-gray-600 dark:text-slate-400">{{ param.type }}</td>
								<td class="py-2 pr-4">
									<span v-if="param.required" class="text-xs px-2 py-0.5 bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-400 rounded">required</span>
									<span v-else class="text-xs px-2 py-0.5 bg-gray-100 text-gray-600 dark:bg-slate-700 dark:text-slate-400 rounded">optional</span>
								</td>
								<td class="py-2 text-gray-600 dark:text-slate-400">{{ param.description }}</td>
							</tr>
						</tbody>
			...
			<div v-if="bodyParams?.length" class="space-y-2">
				...
				<div class="overflow-x-auto">
					<table class="min-w-full text-sm">
						...
						<tbody>
							<tr v-for="param in bodyParams" :key="param.name" class="border-t border-gray-100 dark:border-slate-700">
								<td class="py-2 pr-4">
									<code class="text-purple-600 dark:text-purple-400">{{ param.name }}</code>
								</td>
								<td class="py-2 pr-4 text-gray-600 dark:text-slate-400">{{ param.type }}</td>
