<template>
  <Modal v-model="isOpen" :title="$t('users.addUser')" size="lg" :close-on-overlay="false">
...
const handleSubmit = async () => {
  ...
  try {
    ...
    // Platform admin viewing a partner - add user to that partner
    if (authStore.user?.accountType === 'platform' && partnerStore.selectedPartner) {
      userData.accountType = 'partner'
      userData.accountId = partnerStore.selectedPartner._id
    }
