<template>
	<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden sticky top-4">
		<!-- Header -->
		...
		<div class="p-4 border-b border-gray-200 dark:border-slate-700">
			...
			<div class="grid grid-cols-2 gap-3 text-sm">
				<div>
					...
					<div class="font-medium text-gray-900 dark:text-white">{{ formatDate(search.dateRange.start) }}</div>
				</div>
				<div>
					<span class="text-gray-500 dark:text-slate-400">{{ $t('booking.checkOut') }}</span>
					<div class="font-medium text-gray-900 dark:text-white">{{ formatDate(search.dateRange.end) }}</div>
				</div>
			</div>
