	...
	<div class="space-y-6">
		...
		<div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl p-6 border border-indigo-200 dark:border-indigo-700">
			...
			<div class="grid grid-cols-2 gap-4 text-sm">
				...
				<div class="text-gray-600 dark:text-gray-400">{{ $t('booking.amendment.changesCount') }}</div>
				<div class="font-medium text-gray-900 dark:text-white">
					{{ previewData?.changes?.length || 0 }}
				</div>

				<template v-if="previewData?.priceDifference?.difference !== 0">
					<div class="text-gray-600 dark:text-gray-400">{{ $t('booking.amendment.priceDifference') }}</div>
					<div class="font-medium" :class="previewData?.priceDifference?.isIncrease ? 'text-amber-600' : 'text-green-600'">
				...
				<template v-if="priceAdjustment?.waived">
					<div class="text-gray-600 dark:text-gray-400">{{ $t('booking.amendment.adjustedDifference') }}</div>
		...
		<div v-if="priceAdjustment?.waived || priceAdjustment?.adjustedAmount" class="bg-green-50 dark:bg-green-900/20 rounded-xl p-4 border border-green-200 dark:border-green-700">
			<div class="flex items-center gap-2 text-green-700 dark:text-green-300">
				<span class="material-icons text-sm">check_circle</span>
				<span class="font-medium">
					{{ priceAdjustment?.waived ? $t('booking.amendment.priceWaived') : $t('booking.amendment.priceAdjusted') }}
				</span>
			</div>
			<p v-if="priceAdjustment?.reason" class="mt-2 text-sm text-green-600 dark:text-green-400 ml-6">
...
// Watch for changes
watch(() => props.formData?.reason, (newVal) => {
	reason.value = newVal || ''
})
