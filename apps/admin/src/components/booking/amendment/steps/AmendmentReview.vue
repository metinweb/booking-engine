<template>
	<div class="space-y-6">
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
				...
				<div class="space-y-4">
					...
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<div v-for="(room, i) in originalData?.rooms" :key="i" class="font-medium text-gray-900 dark:text-white">
			...
			<!-- New (After) -->
			<div class="bg-indigo-50 dark:bg-indigo-900/20 rounded-xl p-6 border-2 border-indigo-300 dark:border-indigo-700">
				<div class="flex items-center gap-2 mb-4">
				...
				<div class="space-y-4">
					...
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<p class="font-medium text-gray-900 dark:text-white flex items-center gap-2">
								{{ formatDate(previewData?.preview?.checkIn || formData?.checkIn) }} - {{ formatDate(previewData?.preview?.checkOut || formData?.checkOut) }}
							...
							<p class="text-xs text-indigo-500 dark:text-indigo-400">
								{{ previewData?.preview?.nights || calculatedNights }} {{ $t('booking.nightsUnit') }}
							</p>
						</div>
					</div>
					...
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<div v-for="(room, i) in (previewData?.preview?.rooms || formData?.rooms)" :key="i" class="font-medium text-gray-900 dark:text-white flex items-center gap-2">
								{{ room.roomTypeName?.[locale] || room.roomTypeName?.tr || getSelectedRoomTypeName(room.roomTypeId) }} /
								{{ room.mealPlanName?.[locale] || room.mealPlanName?.tr || getSelectedMealPlanName(room.mealPlanId) }}
								<span v-if="hasRoomChange(i)" class="text-amber-500 material-icons text-sm">bolt</span>
							</div>
						</div>
					</div>
					...
					<div class="flex items-start gap-3">
						...
						<div>
							<p class="text-sm text-indigo-500 dark:text-indigo-400">{{ $t('booking.leadGuest') }}</p>
							<p class="font-medium text-gray-900 dark:text-white flex items-center gap-2">
								{{ formData?.leadGuest?.firstName }} {{ formData?.leadGuest?.lastName }}
								<span v-if="hasGuestChange" class="text-amber-500 material-icons text-sm">bolt</span>
							</p>
						</div>
					</div>

					<!-- Price -->
					<div class="flex items-start gap-3 pt-3 border-t border-indigo-200 dark:border-indigo-700">
						<span class="material-icons text-indigo-400 text-sm mt-1">payments</span>
						<div>
							<p class="text-sm text-indigo-500 dark:text-indigo-400">{{ $t('booking.grandTotal') }}</p>
		...
		<!-- Changes List -->
		<div v-if="previewData?.changes?.length > 0" class="bg-white dark:bg-gray-700 rounded-xl p-6 border border-gray-200 dark:border-gray-600">
		...
		<div v-if="previewData?.priceDifference" class="rounded-xl p-6 border-2" :class="priceDifferenceCardClass">
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-3">
					<div class="w-12 h-12 rounded-full flex items-center justify-center" :class="priceDifferenceIconClass">
...
const getSelectedRoomTypeName = (id) => {
	...
	return typeof name === 'object' ? (name[locale.value] || name.tr || name.en) : (name || roomType.code || id)
}
...
const getSelectedMealPlanName = (id) => {
	...
	return typeof name === 'object' ? (name[locale.value] || name.tr || name.en) : (name || mealPlan.code || id)
}
