<template>
	<div class="space-y-6">
		...
		<template v-else>
		<!-- Hotel Info -->
		<div class="bg-gradient-to-r from-indigo-50 to-purple-50 dark:from-indigo-900/30 dark:to-purple-900/30 rounded-xl p-6">
			<div class="flex items-start gap-4">
				<div class="w-20 h-20 rounded-lg bg-gray-200 dark:bg-gray-700 overflow-hidden flex-shrink-0">
					<img
		...
		<div class="grid grid-cols-1 md:grid-cols-2 gap-6">
			<!-- Dates -->
			<div class="bg-white dark:bg-gray-700 rounded-xl p-5 border border-gray-200 dark:border-gray-600">
				...
				<div class="space-y-2 text-sm">
					...
					<div class="flex justify-between pt-2 border-t dark:border-gray-600">
						...
						<span class="font-semibold text-indigo-600 dark:text-indigo-400">{{ booking?.nights }} {{ $t('booking.nightsUnit') }}</span>
					</div>
				</div>
			</div>
			...
			<div class="bg-white dark:bg-gray-700 rounded-xl p-5 border border-gray-200 dark:border-gray-600">
				...
				<div class="space-y-2 text-sm">
					<div class="flex justify-between">
						...
						<span class="font-medium text-gray-900 dark:text-white">{{ booking?.totalRooms }}</span>
					</div>
					<div class="flex justify-between">
						<span class="text-gray-500 dark:text-gray-400">{{ $t('booking.adults') }}</span>
						<span class="font-medium text-gray-900 dark:text-white">{{ booking?.totalAdults }}</span>
					</div>
					<div class="flex justify-between">
						<span class="text-gray-500 dark:text-gray-400">{{ $t('booking.children') }}</span>
						<span class="font-medium text-gray-900 dark:text-white">{{ booking?.totalChildren || 0 }}</span>
					</div>
				</div>
			</div>
			...
			<div class="bg-white dark:bg-gray-700 rounded-xl p-5 border border-gray-200 dark:border-gray-600">
				...
				<div class="space-y-2 text-sm">
					...
					<div v-if="booking?.leadGuest?.tcNumber" class="flex justify-between">
						...
						<span class="font-medium text-gray-900 dark:text-white">{{ booking?.leadGuest?.tcNumber }}</span>
					</div>
					<div v-if="booking?.contact?.email" class="flex justify-between">
						<span class="text-gray-500 dark:text-gray-400">{{ $t('common.email') }}</span>
						<span class="font-medium text-gray-900 dark:text-white">{{ booking?.contact?.email }}</span>
					</div>
					<div v-if="booking?.contact?.phone" class="flex justify-between">
						<span class="text-gray-500 dark:text-gray-400">{{ $t('common.phone') }}</span>
						<span class="font-medium text-gray-900 dark:text-white">{{ booking?.contact?.phone }}</span>
					</div>
				</div>
			</div>
			...
			<div class="bg-white dark:bg-gray-700 rounded-xl p-5 border border-gray-200 dark:border-gray-600">
				...
				<div class="space-y-2 text-sm">
					...
					<div v-if="booking?.pricing?.totalDiscount > 0" class="flex justify-between text-green-600 dark:text-green-400">
						<span>{{ $t('booking.discount') }}</span>
						<span>-{{ formatCurrency(booking?.pricing?.totalDiscount, booking?.pricing?.currency) }}</span>
					</div>
					<div class="flex justify-between pt-2 border-t dark:border-gray-600">
						<span class="font-medium text-gray-900 dark:text-white">{{ $t('booking.grandTotal') }}</span>
			...
			<div class="space-y-3">
				...
				>
					<div class="flex items-center gap-3">
						<div class="w-8 h-8 rounded-full bg-indigo-100 dark:bg-indigo-900 flex items-center justify-center text-sm font-medium text-indigo-600 dark:text-indigo-400">
							{{ index + 1 }}
						</div>
						<div>
							...
							<p class="text-sm text-gray-500 dark:text-gray-400">
								...
								<span v-if="room.rateType === 'non_refundable'" class="ml-2 text-amber-600 dark:text-amber-400">
