<template>
	<div class="space-y-4">
		...
		<div v-else class="relative">
			...
			>
				<!-- Timeline dot -->
				<div class="absolute left-2 top-0 w-5 h-5 rounded-full flex items-center justify-center"
				...
				<div class="bg-white dark:bg-gray-700 rounded-lg border border-gray-200 dark:border-gray-600 p-4">
					...
					<div class="flex items-start justify-between mb-3">
						<div>
							<span class="px-2 py-0.5 text-xs font-medium rounded-full" :class="getTypeBadgeColor(amendment.type)">
					...
					<div v-if="amendment.changes?.length > 0" class="space-y-2 mb-3">
						<div
							v-for="(change, cIndex) in amendment.changes.slice(0, expandedItems.includes(index) ? undefined : 3)"
							:key="cIndex"
							class="flex items-center gap-2 text-sm"
						>
							<span class="material-icons text-gray-400 text-xs">arrow_right</span>
							<span class="text-gray-600 dark:text-gray-400">{{ change.fieldLabel || change.field }}</span>
						</div>
						<button
						...
						>
							{{ expandedItems.includes(index) ? $t('common.showLess') : `+${amendment.changes.length - 3} ${$t('common.more')}` }}
						</button>
					</div>

					<!-- Price difference -->
					<div v-if="amendment.priceDifference?.difference !== 0" class="flex items-center justify-between pt-3 border-t dark:border-gray-600">
						<span class="text-sm text-gray-500 dark:text-gray-400">{{ $t('booking.amendment.priceDifference') }}</span>
						<div class="text-right">
							<span
								class="font-medium"
								:class="amendment.priceDifference.difference > 0 ? 'text-amber-600' : 'text-green-600'"
							>
								{{ amendment.priceDifference.difference > 0 ? '+' : '' }}{{ formatCurrency(amendment.priceDifference.difference, amendment.priceDifference.currency) }}
							</span>
					...
					>
						<span class="material-icons text-xs">{{ showPreviousState.includes(index) ? 'expand_less' : 'expand_more' }}</span>
						{{ $t('booking.amendment.viewPreviousState') }}
					</button>

					<!-- Previous state details -->
					<div v-if="showPreviousState.includes(index)" class="mt-3 pt-3 border-t dark:border-gray-600 text-xs space-y-1">
						<div class="flex justify-between">
							<span class="text-gray-500 dark:text-gray-400">{{ $t('booking.dates') }}</span>
							<span class="text-gray-700 dark:text-gray-300">
								{{ formatDate(amendment.previousState?.checkIn) }} - {{ formatDate(amendment.previousState?.checkOut) }}
								({{ amendment.previousState?.nights }} {{ $t('booking.nightsUnit') }})
							</span>
						</div>
						<div class="flex justify-between">
							<span class="text-gray-500 dark:text-gray-400">{{ $t('booking.rooms') }}</span>
							<span class="text-gray-700 dark:text-gray-300">{{ amendment.previousState?.totalRooms }}</span>
						</div>
						<div class="flex justify-between">
							<span class="text-gray-500 dark:text-gray-400">{{ $t('booking.grandTotal') }}</span>
							<span class="text-gray-700 dark:text-gray-300">
								{{ formatCurrency(amendment.previousState?.pricing?.grandTotal, amendment.previousState?.pricing?.currency) }}
							</span>
						</div>
					</div>
				</div>
