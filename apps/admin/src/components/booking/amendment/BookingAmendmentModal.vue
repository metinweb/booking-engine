<template>
	<div v-if="show" class="fixed inset-0 z-50 overflow-y-auto">
		...
		<div class="flex min-h-full items-center justify-center p-4">
			<div class="relative bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col transform transition-all">
				<!-- Header -->
						...
						<template v-for="(step, index) in steps" :key="step.key">
							...
							>
								...
								<span
									...
									:class="index === currentStep ? 'text-indigo-600 dark:text-indigo-400' : 'text-gray-500 dark:text-gray-400'"
								>
				...
				<!-- Footer -->
				<div class="flex items-center justify-between px-6 py-4 border-t dark:border-gray-700 bg-gray-50 dark:bg-gray-700/50">
					<button
						v-if="currentStep > 0"
						@click="prevStep"
					...
					<div class="flex items-center gap-3">
						...
						>
							<span v-if="applying" class="animate-spin rounded-full h-4 w-4 border-b-2 border-white mr-2"></span>
...
const canProceed = computed(() => {
	switch (currentStep.value) {
		...
		case 2: // Guests - need lead guest info
			return formData.value.leadGuest?.firstName && formData.value.leadGuest?.lastName
...
const loadBookingData = async () => {
	...
	try {
		...
		console.log('Raw rooms data:', b.rooms?.map(r => ({
			roomType: r.roomType,
		...
		const errorMessages = {
			...
			'BOOKING_CANNOT_BE_AMENDED': 'Bu rezervasyon düzenlenemez. Sadece bekleyen, onaylı veya giriş yapmış rezervasyonlar düzenlenebilir.',
			'PARTNER_CONTEXT_REQUIRED': 'Partner seçimi gerekli'
		}

		const userMessage = errorMessages[errorMessage] || errorMessage || t('booking.amendment.loadError')
...
const checkAvailability = async () => {
	...
	try {
		...
		const errorMessages = {
			...
			'BOOKING_MARKET_NOT_FOUND': 'Rezervasyonun pazarı bulunamadı. Lütfen yönetici ile iletişime geçin.',
