	...
	>
		...
		<div class="bg-gray-50 dark:bg-slate-700/50 rounded-lg p-4 mb-6">
			<div class="flex items-center justify-between">
				<div class="flex items-center gap-6">
					<div>
						<p class="text-xs text-gray-500 dark:text-slate-400 uppercase">{{ $t('payment.total') }}</p>
					...
					<div>
						<p class="text-xs text-gray-500 dark:text-slate-400 uppercase">{{ $t('payment.paid') }}</p>
					...
					<div class="h-8 w-px bg-gray-300 dark:bg-slate-600"></div>
					<div>
						<p class="text-xs text-gray-500 dark:text-slate-400 uppercase">{{ $t('payment.remaining') }}</p>
						<p class="text-lg font-bold" :class="summary.remainingAmount > 0 ? 'text-red-600 dark:text-red-400' : 'text-gray-600 dark:text-slate-300'">
				...
				<div class="flex items-start justify-between">
					<div class="flex items-start gap-3">
						...
						<div>
							...
							<div v-if="payment.type === 'bank_transfer' && payment.bankTransfer" class="text-sm text-gray-500 dark:text-slate-400 mt-1">
								<span v-if="payment.bankTransfer.bankName">{{ payment.bankTransfer.bankName }}</span>
								<span v-if="payment.bankTransfer.reference"> | Ref: {{ payment.bankTransfer.reference }}</span>
							</div>
							<!-- Notes -->
					...
					<div class="flex items-center gap-2">
						...
						<button
							v-if="(payment.type === 'bank_transfer' || payment.type === 'pay_at_checkin') && payment.status === 'pending'"
		...
		>
			...
			<template #footer>
				<div class="flex justify-end gap-3">
					<button @click="showConfirmModal = false" class="btn-secondary px-4 py-2">
						{{ $t('common.cancel') }}
					</button>
					<button @click="confirmPayment" class="btn-primary px-4 py-2" :disabled="confirming">
		...
		>
			...
			<template #footer>
				<div class="flex justify-end gap-3">
					...
					<button @click="deletePayment" class="btn-danger px-4 py-2" :disabled="deleting">
		...
		>
			<div class="space-y-4">
				<div class="p-4 bg-gray-50 dark:bg-slate-700 rounded-lg">
					<p class="text-sm text-gray-500 dark:text-slate-400">{{ $t('payment.refund.originalAmount') }}</p>
					<p class="font-medium text-gray-900 dark:text-white">
						{{ formatPrice(selectedPayment?.amount, selectedPayment?.currency) }}
					</p>
				</div>
			...
			<template #footer>
				<div class="flex justify-end gap-3">
					<button @click="showRefundModal = false" class="btn-secondary px-4 py-2">
						{{ $t('common.cancel') }}
					</button>
					<button
						@click="processRefund"
		...
		>
			<div class="space-y-4">
				...
				<div v-if="selectedPayment?.type === 'bank_transfer'" class="space-y-4 p-4 bg-gray-50 dark:bg-slate-700/50 rounded-lg">
			...
			<template #footer>
				<div class="flex justify-end gap-3">
					<button @click="showEditModal = false" class="btn-secondary px-4 py-2">
						{{ $t('common.cancel') }}
					</button>
					<button @click="updatePayment" class="btn-primary px-4 py-2" :disabled="editing || !editForm.amount">
