		...
		<transition name="modal-fade">
			...
			>
				...
				<div class="relative w-full max-w-lg bg-white dark:bg-slate-800 rounded-xl shadow-2xl overflow-hidden">
					<!-- Header -->
					<div class="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-slate-700 bg-gradient-to-r from-purple-500 to-purple-600">
					...
					<div class="p-5 max-h-[50vh] overflow-y-auto">
						<div class="mb-4">
							<h4 class="text-sm font-medium text-gray-700 dark:text-slate-300 mb-2 flex items-center gap-2">
							...
							<div class="space-y-1.5">
								...
								>
									...
									<div class="flex items-center gap-3">
										<span v-if="day.originalPrice && day.originalPrice > day.price" class="text-xs text-gray-400 line-through">
							...
							<div v-if="!option?.dailyBreakdown?.length" class="text-center text-gray-400 dark:text-slate-500 py-4 text-sm">
						...
						<div v-if="option?.nonRefundable?.enabled" class="border-t border-gray-200 dark:border-slate-700 pt-4 mb-4">
							...
							<div class="grid grid-cols-2 gap-3">
								...
								>
									<div class="flex items-center gap-2 mb-1">
										...
										<span class="text-sm font-semibold text-gray-800 dark:text-white">İade Edilebilir</span>
									</div>
								...
								>
									<div class="flex items-center gap-2 mb-1">
										...
										<span class="text-sm font-semibold text-gray-800 dark:text-white">İade Edilmez</span>
										<span class="px-1.5 py-0.5 text-[10px] font-bold bg-red-100 text-red-600 rounded">
							...
							<!-- Savings Info -->
							<div v-if="selectedRateType === 'non_refundable'" class="mt-2 flex items-center gap-1.5 text-xs text-green-600 dark:text-green-400">
						...
						<div class="border-t border-gray-200 dark:border-slate-700 pt-4 mb-4">
							...
							<div class="flex items-center gap-3">
								...
								<div class="flex rounded-lg bg-gray-100 dark:bg-slate-700 p-0.5">
									...
									>{{ option?.pricing?.currency || 'TRY' }}</button>
								</div>
							...
							<!-- Max Discount Info -->
							<p v-if="maxDiscountPercent" class="mt-2 text-xs text-gray-500 dark:text-slate-400">
								{{ $t('booking.discountFromCommission') }}: {{ $t('common.max') }} {{ maxDiscountPercent }}%
							</p>

							<!-- Discount Warning -->
							<div v-if="isDiscountExceeded" class="mt-2 flex items-center gap-1.5 text-xs text-red-500">
								<span class="material-icons text-sm">warning</span>
								{{ $t('booking.maxDiscountExceeded') }}
							</div>
						</div>
					</div>

					<!-- Footer with Totals -->
					<div class="px-5 py-4 border-t border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50">
						<div class="space-y-2 text-sm">
							...
							<!-- System Discounts -->
							<div v-if="option?.pricing?.totalDiscount > 0" class="flex items-center justify-between text-green-600 dark:text-green-400">
								<span>{{ $t('booking.discount') }} (Kampanyalar)</span>
								<span>-{{ formatPrice(option?.pricing?.totalDiscount) }}</span>
							</div>

							<!-- Custom Discount -->
							<div v-if="calculatedCustomDiscount > 0" class="flex items-center justify-between text-orange-600 dark:text-orange-400">
							...
							<div class="flex items-center justify-between text-xs text-gray-500">
								...
								<span>{{ formatPrice(finalTotal / (option?.nights || 1)) }} / {{ $t('booking.perNight') }}</span>
							</div>

							<!-- Cost & Profit Section (for partner/agency) -->
							<div v-if="costAmount" class="mt-3 pt-3 border-t border-dashed border-gray-300 dark:border-slate-600">
								...
								<div class="flex items-center justify-between mt-1" :class="profit >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'">
									<span class="flex items-center gap-1">
										<span class="material-icons text-sm">{{ profit >= 0 ? 'trending_up' : 'trending_down' }}</span>
										Kâr
									</span>
...
const maxDiscount = computed(() => {
	if (discountType.value === 'percent') {
		return props.maxDiscountPercent
	}
	// Calculate max amount based on percentage
	const basePrice = props.option?.pricing?.finalTotal || 0
	return (basePrice * props.maxDiscountPercent) / 100
})

// Check if discount exceeded
const isDiscountExceeded = computed(() => {
	return discountValue.value > maxDiscount.value
})
...
const handleAddToCart = () => {
	if (isDiscountExceeded.value) return
	...
	emit('add-to-cart', {
		...
		nonRefundableDiscount: selectedRateType.value === 'non_refundable' ? (props.option?.nonRefundable?.discountPercent || 0) : 0,
		customDiscount: {
...
watch(() => props.modelValue, (val) => {
	if (val) {
		...
		// Debug log
		console.log('📊 PriceDetailModal opened with option:', props.option)
		console.log('📊 dailyBreakdown:', props.option?.dailyBreakdown)
		console.log('📊 nonRefundable:', props.option?.nonRefundable)
	}
})
</script>
