<template>
  ...
  >
      ...
      >
        ...
        <div class="flex items-stretch border-b border-gray-200 dark:border-slate-600">
          ...
          <div class="w-32 h-24 flex-shrink-0 relative">
            ...
            <div
              v-else
              class="w-full h-full bg-gray-200 dark:bg-slate-600 flex items-center justify-center"
            >
        ...
        >
          ...
          <span class="text-xs text-amber-700 dark:text-amber-300">{{
            $t('booking.capacityExceeded')
          }}</span>
        </div>

        <!-- Meal Plan Options -->
        <div
          v-if="!room.capacityExceeded && room.options?.length > 0"
              ...
              <div class="flex items-center gap-2 flex-1 min-w-0">
                ...
                >
                  <span class="material-icons text-purple-500 text-sm">{{
                    getMealPlanIcon(option.mealPlan?.code)
                  }}</span>
                </div>
                <div class="min-w-0">
                  <div class="flex items-center gap-1.5">
                    ...
                    <span
                      class="text-xs px-1.5 py-0.5 bg-gray-200 dark:bg-slate-600 text-gray-600 dark:text-slate-300 rounded"
                    >
                  ...
                  <div v-if="option.campaigns?.length > 0" class="flex items-center gap-1 mt-0.5">
                    ...
                    >
                      ...
                      {{
                        ...
                        '-' + Math.round(campaign.discountPercent || 0) + '%'
                      }}
                    </span>
                  </div>
                </div>
        ...
        <div
          v-if="!room.capacityExceeded && room.unavailableOptions?.length > 0"
          class="divide-y divide-gray-200 dark:divide-slate-600 opacity-60"
        >
          <div
          ...
          >
            ...
            <div class="flex items-center gap-2 flex-1 min-w-0">
              <div
                class="w-8 h-8 rounded-lg bg-gray-200 dark:bg-slate-700 flex items-center justify-center flex-shrink-0"
              >
                <span class="material-icons text-gray-400 dark:text-slate-500 text-sm">{{
                  getMealPlanIcon(option.mealPlan?.code)
                }}</span>
              </div>
              <div class="min-w-0">
                <div class="flex items-center gap-1.5">
                  ...
                  <span
                    class="text-xs px-1.5 py-0.5 bg-gray-300 dark:bg-slate-600 text-gray-500 dark:text-slate-400 rounded"
                  >
                    {{ option.mealPlan?.code }}
                  </span>
                </div>
        ...
        <div
          v-if="
            ...
            room.options?.filter(o => o.pricing).length === 0 &&
            !room.unavailableOptions?.length
          "
    ...
    <!-- Room Gallery Modal -->
    <RoomGalleryModal v-model="showRoomGallery" :room-type="selectedRoomType" />
