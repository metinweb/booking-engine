		...
		<div v-if="option" class="space-y-4">
			...
			<div class="bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4">
				<div class="flex items-center justify-between">
					<div>
						<span class="text-sm text-gray-600 dark:text-slate-400">{{ $t('booking.mealPlan') }}</span>
					...
					<div class="text-right">
						<span class="text-sm text-gray-600 dark:text-slate-400">{{ option.nights }} {{ $t('booking.nights') }}</span>
					</div>
				</div>
			</div>
			...
			<!-- Applied Campaigns -->
			<div v-if="option.campaigns?.length > 0" class="bg-green-50 dark:bg-green-900/20 rounded-lg p-4">
			...
			<div class="flex items-center justify-between pt-4 border-t border-gray-200 dark:border-slate-700">
				<div>
					<span class="text-sm text-gray-500 dark:text-slate-400">{{ $t('booking.avgPerNight') }}</span>
					<div class="font-semibold text-gray-900 dark:text-white">
						{{ formatPrice(option.pricing?.avgPerNight, option.pricing?.currency) }}
					</div>
				</div>
				<div class="text-right">
					<span class="text-sm text-gray-500 dark:text-slate-400">{{ $t('booking.grandTotal') }}</span>
