	...
	<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 shadow-lg">
		...
		<div class="p-4 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
			<h3 class="font-semibold text-gray-900 dark:text-white flex items-center">
				...
				{{ $t('booking.cart') }}
				<span class="ml-2 bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-400 px-2 py-0.5 rounded-full text-sm font-medium">
					{{ cart.length }}
				</span>
			</h3>
			<button
		...
		<div v-if="isExpanded && cart.length > 0" class="divide-y divide-gray-100 dark:divide-slate-700 max-h-64 overflow-y-auto">
			<div
			...
			>
				<div class="flex-1 min-w-0">
					...
					<div class="text-xs text-gray-400 dark:text-slate-500">
						...
						<span v-if="item.children?.length">+ {{ item.children.length }} {{ $t('booking.children') }}</span>
					</div>
				</div>
		...
		<!-- Compact Summary -->
		<div v-if="!isExpanded && cart.length > 0" class="px-4 py-2 text-sm text-gray-600 dark:text-slate-400">
