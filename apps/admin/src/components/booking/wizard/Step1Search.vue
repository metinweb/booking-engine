<template>
	<div class="space-y-6">
		...
		<div class="bg-white dark:bg-slate-800 rounded-xl p-6 border border-gray-200 dark:border-slate-700">
			...
			<!-- Selected Hotel Info -->
			<div v-if="selectedHotel" class="mt-4 flex items-start space-x-4 bg-purple-50 dark:bg-purple-900/20 rounded-lg p-4">
				<img
				...
				>
				<div v-else class="w-20 h-20 rounded-lg bg-gray-200 dark:bg-slate-700 flex items-center justify-center flex-shrink-0">
				...
				<div>
					...
					<div v-if="selectedHotel.stars" class="flex items-center mt-1">
						<span v-for="n in selectedHotel.stars" :key="n" class="material-icons text-yellow-400 text-sm">star</span>
					</div>
				</div>
			</div>
		</div>
		...
		<div class="bg-white dark:bg-slate-800 rounded-xl p-6 border border-gray-200 dark:border-slate-700">
			...
			<!-- Date Summary -->
			<div v-if="bookingStore.nights > 0" class="mt-4 flex items-center text-sm text-gray-600 dark:text-slate-400">
		...
		<!-- Error Message -->
		<div v-if="bookingStore.error" class="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 rounded-lg p-4">
...
watch(() => bookingStore.search.hotelId, (newId) => {
