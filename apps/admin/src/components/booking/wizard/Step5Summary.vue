	...
	<div class="space-y-6">
		...
		<div class="bg-purple-50 dark:bg-purple-900/20 rounded-xl p-4 md:p-6">
			<div class="flex items-start space-x-4">
				<div class="w-16 h-16 rounded-lg bg-white dark:bg-slate-800 flex items-center justify-center flex-shrink-0 shadow-sm">
		...
		<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
			...
			<div class="divide-y divide-gray-200 dark:divide-slate-700">
				<div
					v-for="(room, index) in bookingStore.cart"
					:key="index"
					class="p-4"
				>
		...
		<!-- Lead Guest Summary -->
		<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 overflow-hidden">
			<div class="p-4 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-800/50 flex items-center justify-between">
				<h3 class="font-semibold text-gray-900 dark:text-white flex items-center">
		...
		<!-- Special Requests -->
		<div class="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-4 md:p-6">
		...
		<div class="bg-gray-50 dark:bg-slate-800/50 rounded-xl p-4 md:p-6">
			<label class="flex items-start space-x-3 cursor-pointer">
				...
				<span class="text-sm text-gray-600 dark:text-slate-400">
					...
					<a href="#" class="text-purple-600 dark:text-purple-400 hover:underline" @click.prevent="showTerms = true">
						{{ $t('booking.termsAndConditions') }}
					</a>
					{{ $t('booking.and') }}
					<a href="#" class="text-purple-600 dark:text-purple-400 hover:underline" @click.prevent="showPrivacy = true">
		...
		<!-- Navigation -->
		<div class="flex flex-col sm:flex-row justify-between gap-4 pt-4 border-t border-gray-200 dark:border-slate-700">
			<button
				@click="$emit('go-back')"
				class="btn-secondary px-6 py-3 order-2 sm:order-1"
			>
		...
		<Modal v-model="showTerms" :title="$t('booking.termsAndConditions')" size="lg">
			...
			<template #footer>
				<button @click="showTerms = false" class="btn-primary">
					{{ $t('common.close') }}
				</button>
			</template>
		</Modal>
		...
		<Modal v-model="showPrivacy" :title="$t('booking.privacyPolicy')" size="lg">
			...
			<template #footer>
				<button @click="showPrivacy = false" class="btn-primary">
					{{ $t('common.close') }}
				</button>
			</template>
		</Modal>
		...
		<Modal v-model="showSuccess" :title="$t('booking.bookingSuccess')" :closable="false">
			<div class="text-center py-6">
				...
				<div v-if="bookingResult" class="bg-gray-50 dark:bg-slate-800 rounded-lg p-4 mb-4">
					<p class="text-sm text-gray-500 dark:text-slate-400">{{ $t('booking.confirmationNumber') }}</p>
			...
			<template #footer>
				<div class="flex flex-col sm:flex-row gap-3 w-full">
					<button @click="handleViewBooking" class="flex-1 btn-primary">
						{{ $t('booking.viewBooking') }}
					</button>
					<button @click="handleNewBooking" class="flex-1 btn-secondary">
...
const emit = defineEmits(['go-back', 'edit-guests', 'proceed'])
