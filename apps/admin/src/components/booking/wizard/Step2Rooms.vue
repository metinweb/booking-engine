<template>
  <div class="space-y-6">
    ...
    <div v-if="bookingStore.availability.loading" class="flex items-center justify-center py-12">
      <div class="text-center">
        ...
        <p class="mt-3 text-gray-500 dark:text-slate-400">
          {{ $t('booking.searchingAvailability') }}
        </p>
      </div>
    </div>
    ...
    <div v-else-if="bookingStore.availability.results.length === 0" class="text-center py-12">
      ...
      <button class="mt-4 btn-secondary" @click="$emit('go-back')">
    ...
    <div v-else>
      ...
      <div
        ...
        class="md:hidden fixed bottom-0 left-0 right-0 bg-white dark:bg-slate-800 border-t border-gray-200 dark:border-slate-700 p-4 z-30"
      >
        <div class="flex items-center justify-between">
          <div>
            <p class="text-sm text-gray-500 dark:text-slate-400">
              {{ bookingStore.cart.length }} {{ $t('booking.roomsSelected') }}
            </p>
            <p class="text-lg font-bold text-purple-600 dark:text-purple-400">
              {{ formatPrice(bookingStore.grandTotal, bookingStore.currency) }}
            </p>
          </div>
          <button class="btn-primary px-6" @click="$emit('proceed')">
            {{ $t('common.continue') }}
      ...
      <div
        v-if="bookingStore.cart.length > 0"
        class="hidden md:block fixed bottom-6 right-6 w-80 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-200 dark:border-slate-700 p-4 z-30"
      >
        ...
        <div class="mt-4 pt-3 border-t border-gray-200 dark:border-slate-700">
          ...
          <div class="flex space-x-2">
            <button class="flex-1 btn-secondary py-2 text-sm" @click="handleAddAnotherRoom">
              <span class="material-icons text-sm mr-1">add</span>
              {{ $t('booking.addRoom') }}
            </button>
            <button class="flex-1 btn-primary py-2 text-sm" @click="$emit('proceed')">
              {{ $t('common.continue') }}
    ...
    <!-- Error Message -->
    <div
      v-if="bookingStore.availability.error"
