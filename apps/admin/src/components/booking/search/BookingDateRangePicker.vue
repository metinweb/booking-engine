<template>
	<div class="booking-date-picker" ref="containerRef">
		<!-- Display Fields -->
			...
			>
				<div
					...
					:class="[
						...
						checkInDate ? 'border-purple-300 dark:border-purple-700 bg-purple-50/50 dark:bg-purple-900/10' : '',
				...
				>
					...
					<div class="text-left">
						<p class="text-[10px] uppercase tracking-wider text-gray-500 dark:text-slate-400 font-medium">
							{{ $t('booking.checkIn') }}
						</p>
			...
			>
				<div
					...
					:class="[
						...
						checkOutDate ? 'border-purple-300 dark:border-purple-700 bg-purple-50/50 dark:bg-purple-900/10' : '',
		...
		<Teleport to="body">
			<transition name="calendar-fade">
				...
				>
					...
					<div class="px-4 py-3 border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-700/50 rounded-t-2xl">
						<div class="flex items-center justify-between">
							<div class="flex items-center gap-4">
								<!-- Check-in indicator -->
								<div
									class="flex items-center gap-2 px-3 py-1.5 rounded-lg cursor-pointer transition-colors"
									:class="activeField === 'checkin'
								...
								>
									...
									<div class="text-left">
										<p class="text-[10px] uppercase tracking-wider opacity-75">{{ $t('booking.checkIn') }}</p>
										<p class="text-sm font-semibold">{{ checkInDate ? formatShortDate(checkInDate) : '--' }}</p>
									</div>
								</div>
								...
								>
									...
									<div class="text-left">
										<p class="text-[10px] uppercase tracking-wider opacity-75">{{ $t('booking.checkOut') }}</p>
										<p class="text-sm font-semibold">{{ checkOutDate ? formatShortDate(checkOutDate) : '--' }}</p>
									</div>
								</div>

								<!-- Night count in header -->
								<div v-if="nightCount > 0" class="flex items-center gap-1 px-3 py-1.5 bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-300 rounded-lg">
									<span class="material-icons text-sm">nights_stay</span>
									<span class="text-sm font-semibold">{{ nightCount }} {{ nightCount === 1 ? $t('booking.night') : $t('booking.nights') }}</span>
								</div>
							</div>
					...
					<div class="p-4">
						...
						<div class="mt-4 pt-4 border-t border-gray-200 dark:border-slate-700">
							<p class="text-xs text-gray-500 dark:text-slate-400 mb-2">{{ $t('booking.quickSelect') }}</p>
...
const getDateClass = (date) => {
	...
	// Hover range preview
	if (checkInDate.value && !checkOutDate.value && hoverDate.value && activeField.value === 'checkout') {
		const checkIn = new Date(checkInDate.value)
