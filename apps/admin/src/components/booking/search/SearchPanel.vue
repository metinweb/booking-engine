<template>
  ...
  >
    <!-- Provider Tab Bar -->
    <div
      v-if="bookingStore.paximumEnabled"
    ...
    >
      <div class="flex items-center gap-2">
        <span
          ...
          :class="bookingStore.activeProvider === 'paximum' ? 'text-indigo-500' : 'text-purple-500'"
          >search</span
        >
      ...
      <div class="flex items-center gap-2">
        ...
        <div
          v-if="!isExpanded && hasSearchCriteria"
        ...
        >
          <span v-if="bookingStore.search.dateRange.start" class="flex items-center gap-1">
            ...
            {{ formatDateShort(bookingStore.search.dateRange.start) }} -
            {{ formatDateShort(bookingStore.search.dateRange.end) }}
          </span>
          <span class="flex items-center gap-1">
            <span class="material-icons" style="font-size: 12px">people</span>
            {{ bookingStore.search.adults
            }}{{
              bookingStore.search.children.length ? '+' + bookingStore.search.children.length : ''
            }}
          </span>
        </div>
        <button class="text-gray-400 hover:text-gray-600 dark:hover:text-white transition-colors">
          <span class="material-icons transition-transform" :class="{ 'rotate-180': isExpanded }"
            >expand_more</span
          >
        </button>
      </div>
        ...
        <template v-else>
          ...
          <div>
            ...
            <HotelRegionSelector
              ...
              :has-error="locationError"
              @hotel-selected="handleHotelSelected"
            />
            <p v-if="locationError" class="text-red-500 text-xs mt-1">
          ...
          <div class="grid grid-cols-2 gap-3">
            ...
            >
              ...
              <div class="flex items-center gap-2">
                ...
                <span class="w-5 text-center font-semibold text-sm">{{
                  bookingStore.search.adults
                }}</span>
                <button
            ...
            <div
              class="flex items-center justify-between bg-gray-50 dark:bg-slate-700/50 rounded-lg px-3 py-2"
              :class="{ 'opacity-50': !childrenAllowed }"
            >
              <span class="text-xs text-gray-600 dark:text-slate-300">{{
                $t('booking.children')
              }}</span>
              <div class="flex items-center gap-2">
                ...
                <span class="w-5 text-center font-semibold text-sm">{{
                  bookingStore.search.children.length
                }}</span>
                <button
          ...
          <!-- Child Ages -->
          <div
            v-if="childrenAllowed && bookingStore.search.children.length > 0"
          ...
          >
            ...
            >
              ...
              <button class="text-purple-400 hover:text-red-500" @click="removeChild(index)">
          ...
          >
            <span
              v-if="bookingStore.loading.hotels"
              class="material-icons animate-spin mr-1.5 text-sm"
              >refresh</span
            >
...
const hasLocation = computed(() => {
  return (
    selectedLocation.value.hotels.length > 0 ||
...
const canSearch = computed(() => {
  return hasLocation.value && hasDateRange.value && bookingStore.search.adults >= 1
})
...
const handleHotelSelected = hotel => {
  ...
  if (hotel && hotel.maxChildAge > 0) {
    const cappedChildren = bookingStore.search.children.map(age => Math.min(age, hotel.maxChildAge))
