	...
	<div class="bg-white dark:bg-slate-800 rounded-lg border border-gray-200 dark:border-slate-700 overflow-hidden flex-1 flex flex-col min-h-0">
		...
		<div class="px-4 py-3 border-b border-gray-200 dark:border-slate-700 flex items-center justify-between">
			<div class="flex items-center gap-2">
				...
				<span class="font-medium text-gray-900 dark:text-white text-sm">
					...
					<span v-if="unavailableHotels.length > 0 && hotels.length === 0" class="text-gray-400 font-normal">
		...
		<div v-if="loading" class="flex-1 flex items-center justify-center p-8">
			<div class="text-center">
				...
				<p class="text-sm text-gray-500 dark:text-slate-400 mt-2">{{ $t('booking.searchingHotels') }}</p>
			</div>
		</div>

		<!-- Empty State with Unavailable Hotels -->
		<div v-else-if="hotels.length === 0 && unavailableHotels.length > 0" class="flex-1 overflow-y-auto p-3 space-y-2">
			<!-- Info Banner -->
			...
			>
				...
				<div class="relative w-28 h-24 flex-shrink-0 bg-gradient-to-br from-gray-200 to-gray-300 dark:from-slate-600 dark:to-slate-500 flex items-center justify-center">
				...
				<div class="flex-1 p-3 min-w-0 flex flex-col justify-center">
					...
					<div v-if="hotel.priceErrors && hotel.priceErrors.length > 0" class="mt-2">
						...
						>
							<span class="material-icons text-xs">{{ expandedHotels.includes(hotel.hotelId) ? 'expand_less' : 'expand_more' }}</span>
							{{ hotel.roomTypeCount }} {{ $t('booking.roomTypes') }}
						</button>
						<div v-if="expandedHotels.includes(hotel.hotelId)" class="mt-1 text-xs text-gray-500 dark:text-slate-400 pl-4 space-y-0.5">
							<div v-for="err in hotel.priceErrors.slice(0, 5)" :key="err.roomType + err.mealPlan" class="flex items-center gap-1">
		...
		<div v-else-if="hotels.length === 0" class="flex-1 flex items-center justify-center p-8">
			<div class="text-center">
				...
				<p class="text-sm text-gray-500 dark:text-slate-400 mt-2">{{ $t('booking.noHotelsFound') }}</p>
			</div>
		</div>
			...
			<template v-if="unavailableHotels.length > 0">
				<div class="border-t border-gray-200 dark:border-slate-700 pt-3 mt-3">
					...
					>
						...
						<span class="text-xs text-red-500 dark:text-red-400">{{ $t(`booking.issues.${hotel.issues[0]}`) }}</span>
					</div>
				</div>
			</template>
		</div>
