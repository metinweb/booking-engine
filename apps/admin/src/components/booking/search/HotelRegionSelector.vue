<template>
	<div class="relative" ref="containerRef">
		...
		<!-- Search Input -->
		<div class="relative">
			<span class="material-icons absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 text-lg z-10">
			...
			<input
				...
				v-model="searchQuery"
				type="text"
				:placeholder="searchMode === 'hotel' ? $t('booking.hotelSearchPlaceholder') : $t('booking.regionSearchPlaceholder')"
				class="form-input w-full pl-10 pr-10 transition-colors"
				:class="hasError ? 'border-red-500 bg-red-50 dark:bg-red-900/10 focus:border-red-500 focus:ring-red-500' : ''"
				@focus="handleFocus"
				@input="handleInput"
		...
		<div v-if="selectedItems.length > 0" class="flex flex-wrap gap-2 mt-2">
			<span
				...
				:class="item.type === 'hotel'
					? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300'
					: 'bg-blue-100 dark:bg-blue-900/30 text-blue-700 dark:text-blue-300'"
			>
				<span class="material-icons text-xs">{{ item.type === 'hotel' ? 'hotel' : 'location_on' }}</span>
				{{ item.name }}
				<button @click="removeItem(item)" class="hover:text-red-500 ml-1">
		...
		<Teleport to="body">
			<transition name="dropdown-fade">
				...
				>
					...
					<div v-if="!searchQuery && recentItems.length > 0" class="border-b border-gray-200 dark:border-slate-700">
						...
						<div class="max-h-48 overflow-y-auto">
							...
							>
								...
								<div class="flex-1 min-w-0">
									...
									<p v-if="item.subtitle" class="text-xs text-gray-500 dark:text-slate-400 truncate">
										{{ item.subtitle }}
									</p>
								</div>
					...
					<div v-if="searchQuery">
						...
						<div v-else-if="filteredResults.length === 0" class="px-4 py-8 text-center">
							...
							<p class="mt-2 text-sm text-gray-500 dark:text-slate-400">{{ $t('booking.noResultsFound') }}</p>
						</div>
						...
						<div v-else class="max-h-80 overflow-y-auto">
							...
							<template v-if="searchMode === 'hotel' && groupedResults.hotels.length > 0">
								...
								<button
									...
									:key="`hotel-${hotel._id}`"
									type="button"
									@click="selectItem({ id: hotel._id, type: 'hotel', name: hotel.name, stars: hotel.stars, subtitle: hotel.city, data: hotel })"
									@mouseenter="highlightedIndex = index"
									class="w-full px-3 py-2.5 text-left flex items-center gap-3 transition-colors"
									:class="highlightedIndex === index
										? 'bg-purple-50 dark:bg-purple-900/20'
								...
								>
									<!-- Hotel Image -->
									<div class="w-12 h-12 rounded-lg bg-gray-200 dark:bg-slate-600 overflow-hidden flex-shrink-0">
										<img
										...
										/>
										<span v-else class="material-icons text-2xl text-gray-400 dark:text-slate-500 flex items-center justify-center h-full">hotel</span>
									</div>
									<div class="flex-1 min-w-0">
										<p class="text-sm font-medium text-gray-900 dark:text-white truncate">
											{{ hotel.name }}
										...
										<p class="text-xs text-gray-500 dark:text-slate-400 truncate">
											{{ hotel.city }}{{ hotel.tourismRegion ? ` • ${getLocalizedName(hotel.tourismRegion.name)}` : '' }}
										</p>
									</div>
							...
							<template v-if="searchMode === 'region'">
								...
								<template v-if="groupedResults.provinces.length > 0">
									...
									<button
										...
										type="button"
										@click="selectItem({ id: province._id, type: 'province', name: getLocalizedName(province.name), subtitle: `${province.hotelCount} ${$t('booking.hotels').toLowerCase()}`, data: province })"
										@mouseenter="highlightedIndex = index"
										class="w-full px-3 py-2.5 text-left flex items-center gap-3 transition-colors"
										:class="highlightedIndex === index
								...
								<template v-if="groupedResults.regions.length > 0">
									...
									<button
										...
										type="button"
										@click="selectItem({ id: region._id, type: 'region', name: getLocalizedName(region.name), subtitle: `${region.hotelCount} ${$t('booking.hotels').toLowerCase()}`, data: region })"
										@mouseenter="highlightedIndex = groupedResults.provinces.length + index"
										class="w-full px-3 py-2.5 text-left flex items-center gap-3 transition-colors"
										:class="highlightedIndex === groupedResults.provinces.length + index
											? 'bg-blue-50 dark:bg-blue-900/20'
											: 'hover:bg-gray-50 dark:hover:bg-slate-700'"
									>
										...
										<div class="flex-1 min-w-0">
											...
											<p class="text-xs text-gray-500 dark:text-slate-400">
												{{ region.provinceName }} • {{ region.hotelCount }} {{ $t('booking.hotels').toLowerCase() }}
											</p>
										</div>
										<span
					...
					<div v-if="!searchQuery && searchMode === 'hotel' && allHotels.length > 0">
						...
						<div class="max-h-64 overflow-y-auto">
							<button
								...
								class="w-full px-3 py-2.5 text-left flex items-center gap-3 transition-colors"
								:class="highlightedIndex === recentItems.length + index
									? 'bg-purple-50 dark:bg-purple-900/20'
									: 'hover:bg-gray-50 dark:hover:bg-slate-700'"
							>
								<div class="w-10 h-10 rounded-lg bg-gray-200 dark:bg-slate-600 overflow-hidden flex-shrink-0">
									<img
										v-if="hotel.images && hotel.images[0]"
										:src="hotel.images[0].url"
										:alt="hotel.name"
										class="w-full h-full object-cover"
									/>
									<span v-else class="material-icons text-xl text-gray-400 dark:text-slate-500 flex items-center justify-center h-full">hotel</span>
								</div>
								<div class="flex-1 min-w-0">
								...
								<div class="flex flex-col items-end gap-1">
									<span v-if="hotel.stars" class="text-xs text-yellow-500">{{ '★'.repeat(hotel.stars) }}</span>
									<span
										v-if="isItemSelected({ id: hotel._id, type: 'hotel' })"
										class="material-icons text-green-500 text-lg"
									>check_circle</span>
						...
						<div v-if="allRegions.length > 0">
							...
							<div class="max-h-40 overflow-y-auto">
								<button
									...
									class="w-full px-3 py-2.5 text-left flex items-center gap-3 transition-colors"
									:class="highlightedIndex === recentItems.length + allProvinces.length + index
										? 'bg-blue-50 dark:bg-blue-900/20'
										: 'hover:bg-gray-50 dark:hover:bg-slate-700'"
								>
									<span class="material-icons text-lg text-green-500">beach_access</span>
...
const selectedItems = computed(() => {
	...
	if (props.modelValue.hotels) {
		props.modelValue.hotels.forEach(h => {
			...
			if (hotel) {
				items.push({ id: h, type: 'hotel', name: hotel.name, stars: hotel.stars, subtitle: hotel.city })
			}
		})
	}
	if (props.modelValue.regions) {
		props.modelValue.regions.forEach(r => {
			...
			if (region) {
				items.push({ id: r, type: 'region', name: getLocalizedName(region.name), subtitle: region.provinceName })
			}
		})
	}
...
const selectHighlighted = () => {
	if (highlightedIndex.value >= 0 && highlightedIndex.value < filteredResults.value.length) {
		...
		if (item._id) {
			selectItem({
				id: item._id,
				type: searchMode.value === 'hotel' ? 'hotel' : (item.hotelCount !== undefined ? 'region' : 'province'),
