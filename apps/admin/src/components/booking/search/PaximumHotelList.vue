      ...
      <div class="space-y-2">
        ...
        >
          ...
          <div class="relative w-28 h-24 flex-shrink-0">
            ...
            <div
              v-else
              class="w-full h-full bg-gradient-to-br from-gray-100 to-gray-200 dark:from-slate-700 dark:to-slate-600 flex items-center justify-center"
            >
            ...
            <div
              ...
              class="absolute top-1 left-1 w-5 h-5 bg-indigo-500 rounded-full flex items-center justify-center"
            >
          ...
          <div class="flex-1 p-3 min-w-0 flex flex-col justify-center">
            <div class="flex items-start justify-between gap-2">
              <div class="min-w-0 flex-1">
                ...
                <div class="flex items-center gap-2 mt-0.5">
                  <!-- Stars -->
                  <div v-if="hotel.stars > 0" class="flex items-center">
                    <span
                      v-for="n in Math.min(Math.max(0, parseInt(hotel.stars) || 0), 5)"
                      :key="n"
                  ...
                  <!-- Location -->
                  <span
                    v-if="getLocationName(hotel)"
                    class="text-xs text-gray-500 dark:text-slate-400 flex items-center gap-0.5"
                  >
            ...
            <div class="flex items-center justify-between mt-2">
              <!-- Board Type -->
              <span
                v-if="getBoardType(hotel)"
                class="text-xs text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 px-2 py-0.5 rounded"
              >
                {{ getBoardType(hotel) }}
              </span>
              <span v-else></span>
